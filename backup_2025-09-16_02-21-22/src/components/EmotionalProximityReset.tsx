...
import { X, Play, Pause, RotateCcw, Shield, Heart, Eye, Hand, Brain, 
         CheckCircle, AlertTriangle, Users, Calendar, Phone, BookOpen,
         Award, Anchor, RefreshCw, ChevronRight, Timer, HelpCircle } from 'lucide-react';
...
type ContentType = 'trauma' | 'medical' | 'emotional' | 'confrontation' | 'bad-news' | 'other';
type SupportNeed = 'none' | 'break' | 'colleague' | 'supervision' | 'end-day' | 'extended';
...
const resetSteps = [
  ...
  },
  {
    ...
    icon: Eye,
    options: [
      { sense: 'Look at', action: 'Something neutral (wall, floor, your hands)' },
      { sense: 'Touch', action: 'A smooth object, your own arms, a cool surface' },
...
export const EmotionalProximityReset: React.FC<EmotionalProximityResetProps> = ({ 
  onClose, 
  onComplete 
}) => {
  // Core states
  const [phase, setPhase] = useState<'setup' | 'practice' | 'checkin' | 'support'>('setup');
  ...
  const [settingType, setSettingType] = useState<'private' | 'team' | 'public' | 'remote'>('private');
    ...
    if (isPlaying && phase === 'practice') {
      intervalRef.current = setInterval(() => {
        ...
        setStepTimeElapsed(prev => {
          const next = prev + 1;
          const currentStepDuration = quickMode ? 5 : 
            extendedMode ? resetSteps[currentStep].duration * 2 : 
  ...
  }, [isPlaying, phase, currentStep, quickMode, extendedMode, vibrationEnabled]);
  ...
  const handleSubmit = () => {
    ...
    if (logReset) {
      const sessions = JSON.parse(localStorage.getItem('emotionalProximitySessions') || '[]');
      sessions.push(data);
      localStorage.setItem('emotionalProximitySessions', JSON.stringify(sessions));
      ...
      if (recentSessions.length > 10) {
        console.warn('High frequency of emotional proximity resets - consider supervision');
      }
    }
    
    // Flag concerning content impact
    if (contentImpact && (contentImpact.length > 100 || 
        contentImpact.toLowerCase().includes('help') || 
        contentImpact.toLowerCase().includes('can\'t'))) {
      console.warn('Content impact may need additional support:', contentImpact);
    }
    
    // Save preferences
    localStorage.setItem('emotionalProximityPrefs', JSON.stringify({
  ...
  const getStepProgress = () => {
    const duration = quickMode ? 5 : 
      extendedMode ? getCurrentStepInfo().duration * 2 : 
          ...
          <div className="p-8">
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className={`${getTextSize()} ${highContrastMode ? 'text-black' : 'text-gray-600'}`}>
            ...
            <div className={`mb-8 p-6 rounded-xl ${
              ...
              <p className={`${getTextSize()} ${highContrastMode ? 'text-black' : 'text-amber-900'}`}>
                You've just interpreted trauma, medical emergency, emotional content, or difficult material
              </p>
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  { id: 'emotional' as ContentType, label: 'Emotional breakdown' },
                  { id: 'confrontation' as ContentType, label: 'Angry confrontation' },
                  { id: 'bad-news' as ContentType, label: 'Delivering bad news' },
                  { id: 'other' as ContentType, label: 'Other difficult content' }
                ].map(type => (
                  <button
                    key={type.id}
            ...
            <div className={`mb-8 p-4 rounded-lg ${
              highContrastMode ? 'bg-gray-100 border-2 border-black' : 'bg-blue-50'
            }`}>
              <p className={`${getTextSize()} font-medium mb-3`}>
                Your Professional Reset Path:
              </p>
              <div className="flex items-center justify-center gap-2 flex-wrap">
                {['Acknowledge', 'Separate', 'Ground', 'Restore', 'Ready'].map((step, i) => (
                  <React.Fragment key={step}>
                    <span className={`px-3 py-1 rounded ${
                      highContrastMode ? 'bg-white border border-black' : 'bg-white'
                    }`}>
                      {step}
                    </span>
              ...
              <div className="space-y-2">
                <label className="flex items-center">
                  ...
                  <span className={getTextSize()}>
                    <strong>Quick (30 seconds)</strong> - Time pressure or team setting
                  </span>
                </label>
                <label className="flex items-center">
                  <input
                  ...
                  />
                  <span className={getTextSize()}>
                    <strong>Standard (90 seconds)</strong> - Full five-step process
                  </span>
                </label>
                <label className="flex items-center">
                  <input
            ...
            <div className={`mb-8 p-4 rounded-lg ${
              ...
              <ul className={`${getTextSize()} space-y-1 ${
                ...
                <li>• This is professional maintenance, not personal failure</li>
              </ul>
            </div>
            ...
            <div className="mb-8 space-y-2">
              ...
              <label className="flex items-center">
                <input
                  ...
                  className="mr-3"
                />
                <span className={getTextSize()}>Vibration cues for transitions</span>
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
          ...
          <div className="p-8">
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className={`${getTextSize()} ${highContrastMode ? 'text-black' : 'text-gray-600'}`}>
                  {quickMode ? '5' : extendedMode ? currentStepInfo.duration * 2 : currentStepInfo.duration} seconds
                </p>
              </div>
              <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg">
                <X className="w-6 h-6" />
            ...
            {currentStep === 0 && (
              <div className={`mb-6 p-4 rounded-lg text-center ${
                ...
                <p className={`${getTextSize()} font-medium ${
            ...
            <div className={`mb-8 p-8 rounded-xl text-center ${
                ...
                <div className="space-y-3">
                  {currentStepInfo.instructions.map((instruction, i) => (
                    <p key={i} className={`${getTextSize()}`}>
                      {quickMode && i === 0 ? instruction : !quickMode && instruction}
                    </p>
                  ))}
                </div>
              )}
              ...
              {currentStepInfo.id === 'clarity' && (
                <div>
                  <p className={`text-xl font-medium mb-4`}>
                    Say one of these {internalizationMode ? 'internally' : 'aloud'}:
                  </p>
                  <div className="space-y-2">
                    {(quickMode ? [currentStepInfo.statements[0]] : currentStepInfo.statements).map((statement, i) => (
              ...
              {currentStepInfo.id === 'pride' && (
                <div>
                  ...
                  <div className="space-y-2">
                    {(quickMode ? [currentStepInfo.acknowledgments[4]] : currentStepInfo.acknowledgments).map((ack, i) => (
                      <p key={i} className={`${getTextSize()} italic`}>
                        "{ack}"
                      </p>
                    ))}
              ...
              {currentStepInfo.id === 'transition' && (
                <div>
                  ...
                  <div className="space-y-2">
                    {(quickMode ? [currentStepInfo.rituals[1]] : currentStepInfo.rituals).map((ritual, i) => (
                      <p key={i} className={`${getTextSize()}`}>
                        {ritual}
                      </p>
                    ))}
            ...
            <div className="mb-6">
              <div className="flex justify-between mb-2">
                <span className={`${getTextSize()} font-medium`}>Step Progress:</span>
                <span className={getTextSize()}>
                  {stepTimeElapsed}s / {quickMode ? 5 : extendedMode ? currentStepInfo.duration * 2 : currentStepInfo.duration}s
                </span>
              </div>
                ...
                {resetSteps.map((step, i) => (
                  <div
                    ...
                    className={`w-12 h-2 rounded ${
                      i < currentStep 
                        ? (highContrastMode ? 'bg-black' : 'bg-green-600')
                        : i === currentStep
                          ? (highContrastMode ? 'bg-gray-600' : 'bg-blue-600')
                          : (highContrastMode ? 'bg-gray-300' : 'bg-gray-300')
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="flex justify-center gap-3">
              ...
              <button
                onClick={() => setCurrentStep(Math.min(currentStep + 1, resetSteps.length - 1))}
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className={`rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto ${
          ...
          <div className="p-8">
            ...
            <p className={`mb-8 ${getTextSize()} ${highContrastMode ? 'text-black' : 'text-gray-600'}`}>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-4">
                {[
                  { label: 'Emotional distance', value: emotionalDistance, setter: setEmotionalDistance },
                  { label: 'Professional identity', value: professionalIdentity, setter: setProfessionalIdentity },
                  { label: 'Ready for next', value: readinessLevel, setter: setReadinessLevel }
                ].map(scale => (
                  <div key={scale.label}>
                    <div className="flex justify-between mb-2">
                      <span className={getTextSize()}>{scale.label}:</span>
                      <span className={`${getTextSize()} font-medium`}>{scale.value}/10</span>
                    </div>
                    <input
                      type="range"
                      min="1"
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className={`${getTextSize()} font-medium`}>
                      {supportNeed <= 3 ? 'High' : supportNeed >= 8 ? 'Low' : 'Moderate'}
                    </span>
                  </div>
                  <input
                    type="range"
                    ...
                    onChange={(e) => setSupportNeed(11 - parseInt(e.target.value))}
                    className="w-full"
                  />
            ...
            <div className={`mb-6 p-4 rounded-lg ${
              ...
              <p className={`text-sm mb-2 ${highContrastMode ? 'text-black' : 'text-amber-800'}`}>
                Is there anything from this interpretation that needs additional attention?
              </p>
              <textarea
                value={contentImpact}
            ...
            <div className="mb-6">
              <label className={`block mb-2 ${getTextSize()}`}>
                Optional: What helped you maintain professionalism during the difficult interpretation?
              </label>
              <textarea
              ...
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-3"
                  />
                  <span className={getTextSize()}>Log this reset (tracks cumulative exposure)</span>
                </label>
                <label className="flex items-center">
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-3"
                  />
                  <span className={getTextSize()}>Share strategy with team (anonymous)</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  if (supportNeed <= 3 || separationLevel === 'Need additional support') {
                    setPhase('support');
                  } else {
                    handleSubmit();
                  }
      ...
      <div className={`rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto ${
        ...
        <div className="p-8">
          ...
          <div className={`mb-6 p-6 rounded-xl ${
            highContrastMode ? 'bg-blue-100 border-2 border-black' : 'bg-blue-50'
          }`}>
            <h3 className={`font-semibold mb-3 flex items-center gap-2 ${getTextSize()}`}>
          ...
          <div className={`mb-6 p-6 rounded-xl ${
            ...
            <div className="space-y-3">
              <button className={`w-full text-left p-3 rounded-lg flex items-center gap-3 ${
                highContrastMode ? 'bg-white border border-black hover:bg-gray-100' : 'bg-white hover:bg-gray-50'
              }`}>
              ...
              <button className={`w-full text-left p-3 rounded-lg flex items-center gap-3 ${
                ...
                <div>
                  ...
                  <p className="text-sm opacity-75">Access professional support</p>
                </div>
              </button>
          ...
          <div className={`mb-8 p-4 rounded-lg ${
            ...
            <p className={`${getTextSize()} ${
              highContrastMode ? 'text-black' : 'text-green-800'
            }`}>
              Secondary trauma is real. Seeking support is professional, not personal weakness. 
              Your wellbeing directly impacts your ability to serve effectively.
            </p>
          </div>
