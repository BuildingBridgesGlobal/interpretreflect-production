...
import { HeartPulseIcon, SecureLockIcon, TargetIcon, CommunityIcon, NotepadIcon } from './CustomIcon';
...
Generated by InterpretReflect™`;
            ...
            <li key={step.id} className="flex-1">
              <button
                ...
                className={`w-full flex flex-col items-center p-3 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  isCompleted || isCurrent ? 'text-white shadow-md hover:shadow-lg' : 'bg-gray-50'
                }`}
                style={{ 
                  background: isCompleted || isCurrent ? 'linear-gradient(135deg, #1b5e20, #2e7d32)' : 'transparent',
              ...
              >
                ...
                <span
                  className={`text-xs ${
                    isCompleted ? 'font-semibold' : isCurrent ? 'font-medium' : ''
                  }`}
                  style={{
  ...
  const renderAssignmentDetails = () => (
    <section aria-labelledby="assignment-heading">
      <h3 id="assignment-heading" className="text-xl font-bold mb-4" style={{ color: '#2D3748' }}>
        Assignment Details
      </h3>
            ...
            return (
              <label
                ...
                style={{
                  ...
                  borderColor: formData.assignmentType === type.value ? '#5C7F4F' : undefined,
                  focusRingColor: '#5C7F4F'
                }}
              >
                <input
          ...
          {['30 min', '1 hour', '2 hours', '3 hours', '4+ hours', 'All day'].map(duration => (
            <label
              ...
              style={{
                ...
                borderColor: formData.duration === duration ? '#5C7F4F' : undefined,
                focusRingColor: '#5C7F4F'
              }}
            >
              <input
  ...
  const renderEmotionalCheck = () => (
    <section aria-labelledby="emotional-heading">
          ...
          {EMOTIONAL_STATES.map(state => (
            <label
              ...
              style={{
                ...
                borderColor: formData.emotionalState === state.value ? '#5C7F4F' : undefined,
                focusRingColor: '#5C7F4F'
              }}
            >
              <input
      ...
      {showComparison && previousPrep && (
        ...
        >
          <h4 id="comparison-heading" className="text-sm font-medium mb-2" style={{ color: '#2D3748' }}>
        ...
        <div className="space-y-3">
          {PHYSICAL_STATES.map(state => (
            <label
              ...
              style={{
                backgroundColor: formData.physicalReadiness === state.value ? '#F0F5ED' : '#FFFFFF',
                borderColor: formData.physicalReadiness === state.value ? '#5C7F4F' : undefined,
                focusRingColor: '#5C7F4F'
              }}
            >
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, physicalReadiness: e.target.value }))}
                className="absolute opacity-0 pointer-events-none"
              />
              <span className="flex items-center gap-3">
                <span className="text-2xl" aria-hidden="true">{state.emoji}</span>
              ...
              <Circle
                className={`h-5 w-5 ${
                  formData.physicalReadiness === state.value ? 'fill-current' : ''
                }`}
                style={{ color: formData.physicalReadiness === state.value ? '#5C7F4F' : '#CBD5E0' }}
              />
            </label>
          ))}
        </div>
  ...
  const renderKnowledgePrep = () => (
    <section aria-labelledby="knowledge-heading">
      <h3 id="knowledge-heading" className="text-xl font-bold mb-4" style={{ color: '#2D3748' }}>
  ...
  const renderSupportNetwork = () => (
    <section aria-labelledby="support-heading">
      <h3 id="support-heading" className="text-xl font-bold mb-4" style={{ color: '#2D3748' }}>
        Support Network
      </h3>
      <p className="text-sm mb-6" style={{ color: '#4A5568' }}>
  ...
  const renderSelfCarePlan = () => (
    <section aria-labelledby="selfcare-heading">
      ...
      <fieldset>
        ...
        {['Take a walk', 'Practice breathing exercises', 'Call a friend', 'Rest', 'Stretch', 'Hydrate'].map(activity => (
          <label
            key={activity}
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.checked) {
                  ...
                  setFormData(prev => ({
                    ...prev,
                    selfCarePost: prev.selfCarePost.filter(a => a !== activity)
                  }));
                }
              }}
  ...
  const renderPositiveMindset = () => (
    <section aria-labelledby="mindset-heading">
      <h3 id="mindset-heading" className="text-xl font-bold mb-4" style={{ color: '#2D3748' }}>
        Positive Mindset
      </h3>
      <p className="text-sm mb-6" style={{ color: '#4A5568' }}>
  ...
  const renderReviewStep = () => (
    <section aria-labelledby="review-heading">
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: '#F7FAFC' }}>
          <div className="flex justify-between items-start mb-2">
            <h4 className="font-medium" style={{ color: '#2D3748' }}>Assignment Details</h4>
            <button
          ...
          <ul className="space-y-1 text-sm" style={{ color: '#4A5568' }}>
            <li>Type: <strong>{formData.assignmentType || 'Not specified'}</strong></li>
            <li>Duration: <strong>{formData.duration || 'Not specified'}</strong></li>
          </ul>
        </div>
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: '#F7FAFC' }}>
          <div className="flex justify-between items-start mb-2">
            <h4 className="font-medium" style={{ color: '#2D3748' }}>Emotional Check</h4>
            <button
          ...
          <p className="text-sm" style={{ color: '#4A5568' }}>
            Feeling: <strong>{formData.emotionalState || 'Not specified'}</strong>
          </p>
        </div>
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: '#F7FAFC' }}>
          <div className="flex justify-between items-start mb-2">
            <h4 className="font-medium" style={{ color: '#2D3748' }}>Physical Readiness</h4>
            <button
          ...
          <p className="text-sm" style={{ color: '#4A5568' }}>
            State: <strong>{formData.physicalReadiness || 'Not specified'}</strong>
          </p>
        </div>
        ...
        {formData.knowledgeGaps.length > 0 && (
          <div className="p-4 rounded-lg" style={{ backgroundColor: '#F7FAFC' }}>
            <div className="flex justify-between items-start mb-2">
              <h4 className="font-medium" style={{ color: '#2D3748' }}>Knowledge Areas to Review</h4>
              <button
        ...
        {formData.selfCarePost.length > 0 && (
          <div className="p-4 rounded-lg" style={{ backgroundColor: '#F7FAFC' }}>
            <div className="flex justify-between items-start mb-2">
              <h4 className="font-medium" style={{ color: '#2D3748' }}>Post-Assignment Self-Care</h4>
              <button
      ...
      >
        <h4 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: '#5C7F4F' }}>
