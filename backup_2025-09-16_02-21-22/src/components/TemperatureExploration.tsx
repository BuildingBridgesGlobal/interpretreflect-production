...
import { X, Play, Pause, RotateCcw, Droplets, Sun, Wind, Brain, Eye, Hand, Clock, Shield, Sparkles, 
         Thermometer, Contrast, Vibrate, Timer, Settings, ChevronRight, Heart, Circle } from 'lucide-react';
...
type ExplorationMethod = 'physical' | 'imaginative' | 'breath' | 'sensory' | 'none';
type IntensityLevel = 'gentle' | 'moderate' | 'intense' | 'none';
type DurationOption = '10s' | '30s' | '1m' | '2m' | 'complete';

export const TemperatureExploration: React.FC<TemperatureExplorationProps> = ({ onClose, onComplete }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentPhase, setCurrentPhase] = useState<'exploring' | 'resting' | 'stopped'>('stopped');
  const [timeElapsed, setTimeElapsed] = useState(0);
  const [selectedApproach, setSelectedApproach] = useState<ApproachType>('neutral');
  const [selectedMethod, setSelectedMethod] = useState<ExplorationMethod>('none');
  const [selectedDuration, setSelectedDuration] = useState<DurationOption>('30s');
  ...
  const [routineMode, setRoutineMode] = useState<'predictable' | 'experimenting' | 'minimal' | 'none'>('none');
  const [sensoryPreference, setensoryPreference] = useState<'seeking' | 'avoiding' | 'balanced'>('balanced');
  const [intensityPreference, setIntensityPreference] = useState<IntensityLevel>('moderate');
  ...
  const getPhaseColor = () => {
    if (highContrastMode) {
      return selectedApproach === 'cool' ? '#000080' :
             selectedApproach === 'warm' ? '#800000' :
             '#404040';
    }
    return selectedApproach === 'cool' ? '#3B82F6' :
           selectedApproach === 'warm' ? '#EF4444' :
           '#6B7280';
  };
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className={`rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${
          ...
          <div className="p-8">
            <h2 className={`text-2xl font-bold mb-2 ${highContrastMode ? 'text-black' : 'text-gray-900'}`}>
              Your Temperature Experience
            </h2>
            <p className="text-gray-600 mb-6">Every response helps us understand what works</p>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setExploredToday(exploredToday.filter(o => o !== option));
                        }
                      }}
                      className="mr-3"
                    />
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setHelpfulAspects(helpfulAspects.filter(a => a !== aspect));
                        }
                      }}
                      className="mr-3"
                    />
            ...
            {/* Sensory needs */}
            <div className="mb-6">
              <p className="font-semibold mb-2">Optional: Any sensory needs we should know about?</p>
              <textarea
                value={sensoryNeeds}
        ...
        <div className="p-8">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-gray-600">Work with temperature in your own way</p>
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[
                { id: 'cool' as ApproachType, label: 'Cool Options', icon: Droplets, color: '#3B82F6', desc: 'Water • Air • Object • Imagination' },
                { id: 'warm' as ApproachType, label: 'Warm Options', icon: Sun, color: '#EF4444', desc: 'Heat • Sunlight • Layers • Visualization' },
                { id: 'neutral' as ApproachType, label: 'Neutral Options', icon: Circle, color: '#6B7280', desc: 'Room temp • No change • Observation' },
                { id: 'contrast' as ApproachType, label: 'Contrast', icon: Thermometer, color: '#8B5CF6', desc: 'Alternate temperatures' },
                { id: 'imaginative' as ApproachType, label: 'Imaginative', icon: Brain, color: '#10B981', desc: 'Visualization only' }
              ].map(approach => {
                  ...
                  <button
                    ...
                    className={`p-4 rounded-lg border text-center transition-all ${
                      selectedApproach === approach.id 
                        ? (highContrastMode ? 'bg-black text-white border-black' : 'bg-sage-50 border-sage-500')
                        : 'border-gray-300 hover:bg-gray-50'
                    }`}
                  >
                    <Icon className="w-6 h-6 mx-auto mb-2" style={{ 
                      color: selectedApproach === approach.id ? (highContrastMode ? '#FFFFFF' : approach.color) : '#9CA3AF' 
                    }} />
          ...
          <div className={`mb-6 p-6 rounded-xl ${
            ...
            <div className="text-center">
              ...
              <h3 className="text-3xl font-bold mb-2 capitalize text-white">
                {currentPhase === 'stopped' ? 'Begin when comfortable' : 
                 currentPhase === 'exploring' ? 'Exploring Temperature' : 'Resting'}
              </h3>
              {isPlaying && (
              ...
              <div className="mt-4">
                <div className="h-3 bg-black bg-opacity-20 rounded-full overflow-hidden border border-white border-opacity-50">
                  <div 
                    ...
                    style={{
                      width: selectedDuration === 'complete' ? '0%' :
                             `${(timeElapsed / ({
                               '10s': 10,
                               '30s': 30,
                               '1m': 60,
                ...
                <p className={`text-center mt-2 ${highContrastMode ? 'text-white' : 'text-white'}`}>
          ...
          {!simplifiedMode && (
            <div className="mb-6">
              <p className="font-semibold mb-3">Ways to Explore (all equally valid):</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className={`p-4 rounded-lg ${
                  ...
                  <ul className="text-sm space-y-1">
                    ...
                    <li>• Warm: Warm compress, sunshine, heated object, layers</li>
          ...
          {neurodivergentMode && (
            <div className={`mb-6 p-4 rounded-lg ${
              highContrastMode ? 'bg-black text-white border-2 border-white' : 'bg-purple-50 border border-purple-200'
            }`}>
              <h4 className="font-semibold mb-2 flex items-center">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <label className="flex items-center">
                  <input 
                    ...
                    onChange={(e) => setSensoryPreference(e.target.checked ? 'seeking' : 'balanced')}
                    className="mr-2"
                  />
                ...
                <label className="flex items-center">
                  <input 
                    ...
                    onChange={(e) => setSensoryPreference(e.target.checked ? 'avoiding' : 'balanced')}
                    className="mr-2"
                  />
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              {(['10s', '30s', '1m', '2m', 'complete'] as DurationOption[]).map(duration => (
                <button
                  key={duration}
                ...
                >
                  {duration === 'complete' ? 'Until it feels complete' : duration}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Safety & Comfort */}
          <div className={`mb-6 p-4 rounded-lg ${
            highContrastMode ? 'bg-yellow-100 text-black border-2 border-black' : 'bg-yellow-50 border border-yellow-200'
          }`}>
            <h4 className="font-semibold mb-2 flex items-center">
          ...
          <div className="flex justify-center gap-2 mb-6">
            <button
              onClick={() => {
                ...
                localStorage.setItem('temperaturePrefs', JSON.stringify({
                  ...JSON.parse(localStorage.getItem('temperaturePrefs') || '{}'),
                  highContrast: !highContrastMode
                }));
              }}
            ...
            {!isPlaying ? (
              <button
                ...
                className={`flex items-center px-8 py-3 rounded-lg font-semibold ${
                  highContrastMode ? 'bg-black text-white' : 'bg-sage-600 text-white hover:bg-sage-700'
                }`}
              >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setIntensityPreference(e.target.value as IntensityLevel)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSensoryPreference(e.target.value as any)}
                    className={`w-full px-3 py-2 rounded-lg border ${
                      highContrastMode ? 'bg-black text-white border-white' : 'border-gray-300'
                    }`}
                  >
                    <option value="seeking">Sensory Seeking (more input)</option>
                    <option value="avoiding">Sensory Avoiding (less input)</option>
                ...
                <button
                  ...
                  className={`w-full px-4 py-2 rounded-lg font-semibold ${
                    highContrastMode ? 'bg-white text-black' : 'bg-sage-600 text-white'
                  }`}
                >
                  Save Settings
                </button>
