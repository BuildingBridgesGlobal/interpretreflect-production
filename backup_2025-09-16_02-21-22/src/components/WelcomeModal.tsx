...
import { X, ChevronRight, Clock, Heart, Users, Sparkles } from 'lucide-react';
import { HeartPulseIcon, NotepadIcon, HourglassPersonIcon, TargetIcon, CommunityIcon } from './CustomIcon';
...
export const WelcomeModal: React.FC<WelcomeModalProps> = ({ onClose, onComplete }) => {
  ...
  const questions = [
    {
      ...
      options: [
        ...
        { value: 'post-assignment', label: 'Processing after an assignment', icon: <TargetIcon size={32} /> },
        { value: 'team', label: 'Team or mentoring support', icon: <CommunityIcon size={32} /> },
        { value: 'wellness', label: 'Personal wellness & self-care', icon: <HeartPulseIcon size={32} /> }
      ]
    },
    {
      id: 'timeAvailable',
      ...
      options: [
        { value: '3min', label: '3 minutes', icon: <Clock className="w-8 h-8" /> },
        { value: '10min', label: '10 minutes', icon: <Clock className="w-8 h-8" /> },
        { value: '20min', label: '20+ minutes', icon: <Clock className="w-8 h-8" /> },
        { value: 'flexible', label: "I'm flexible", icon: <Clock className="w-8 h-8" /> }
      ]
    },
    {
      id: 'energyLevel',
      ...
      options: [
        ...
        { value: 'balanced', label: 'Feeling balanced', icon: <Heart className="w-8 h-8" /> },
        { value: 'high', label: 'Ready to reflect deeply', icon: <Heart className="w-8 h-8" /> },
        { value: 'stressed', label: 'Feeling stressed or overwhelmed', icon: <Heart className="w-8 h-8" /> }
      ]
    }
  ];
  ...
  const getRecommendations = () => {
    ...
    } else if (answers.timeAvailable === '20min' || answers.timeAvailable === 'flexible') {
    ...
    if (answers.energyLevel === 'stressed' && !recs.includes('Stress Reset Tool')) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="px-8 pt-8 pb-4 border-b" style={{ backgroundColor: '#FAF9F6' }}>
