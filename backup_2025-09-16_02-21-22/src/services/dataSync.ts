...
export class DataSyncService {
  ...
  async syncAllData(): Promise<SyncStatus> {
    ...
    try {
      ...
      console.log(`Data sync completed: ${results.synced} items synced, ${results.failed} failed`);
  ...
  private async syncBreathingPreferences(userId: string): Promise<SyncStatus> {
    try {
      const breathingPrefs = localStorage.getItem('breathingAccessibilityPrefs');
  ...
  private async syncBodyAwarenessSessions(userId: string): Promise<SyncStatus> {
    try {
      ...
      return { success: false, error: 'Failed to sync body awareness sessions' };
    }
  }
  ...
  private async syncAssignmentData(userId: string): Promise<SyncStatus> {
    try {
      ...
      const postAssignmentDebriefs = localStorage.getItem('postAssignmentDebriefs');
  ...
  private async syncEmotionalProximityData(userId: string): Promise<SyncStatus> {
    try {
      ...
      return { success: false, error: 'Failed to sync emotional proximity data' };
    }
  }
  ...
  private async syncTemperaturePreferences(userId: string): Promise<SyncStatus> {
    try {
      ...
      return { success: false, error: 'Failed to sync temperature preferences' };
    }
  }
