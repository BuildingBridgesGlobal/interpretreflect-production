...
export interface EmotionalContagionEvent {
  ...
  emotion_category: 'positive_high_energy' | 'positive_low_energy' | 
                   'negative_high_energy' | 'negative_low_energy' | 'neutral';
...
export async function generateEmotionalWeatherMap(
  ...
  try {
    const { data, error } = await supabase.rpc('generate_emotional_weather_map', {
      p_org_id: orgId,
...
export async function identifyPositiveInfluencers(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('identify_positive_influencers', {
      p_team_hash: teamHash,
      p_days: days
    });
...
export async function analyzeContagionResearch(
  ...
  try {
    const { data, error } = await supabase.rpc('analyze_contagion_research_data', {
    ...
    subscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (event.emotion_category.startsWith('negative') && 
            ...
            callback({
              ...
              data: {
                ...
                message: 'Negative emotion spreading rapidly - intervention recommended'
              }
            });
...
export async function triggerContagionIntervention(
  teamId: string,
  interventionType: 'wellness_break' | 'team_huddle' | 'individual_support' | 'positive_activity'
): Promise<{ success: boolean; error?: string }> {
  try {
    const teamHash = await createHash(teamId);
...
async function getCurrentContext(): Promise<EmotionalContagionEvent['context_type']> {
...
async function getWorkloadIntensity(): Promise<'light' | 'moderate' | 'heavy' | 'critical'> {
