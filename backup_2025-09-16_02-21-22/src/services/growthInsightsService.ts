...
export async function updateGrowthInsightsForUser(
  ...
  try {
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to update growth insights' 
    };
  }
}
 ...
 */
function analyzePreAssignmentAnswers(answers: Record<string, string>): GrowthMetrics {
  ...
  if (answers.triggers_vulnerabilities?.length > 50) metrics.self_awareness_level += 40;
  if (answers.anticipated_demands?.length > 50) metrics.self_awareness_level += 20;

  // Role Clarity Score
  if (answers.role_space?.length > 100) {
    metrics.role_clarity_score = Math.min(100, 
  ...
  if (answers.ethics_culture?.length > 50) {
    const hasEthicalConcerns = answers.ethics_culture.toLowerCase().includes('concern') ||
                               answers.ethics_culture.toLowerCase().includes('dilemma');
    metrics.ethical_awareness_score = hasEthicalConcerns ? 80 : 60;
    if (answers.ethics_culture.length > 100) metrics.ethical_awareness_score += 20;
  }

  // Growth Mindset Score
  if (answers.growth_goals?.length > 50) {
    metrics.growth_mindset_score = 50;
    if (answers.growth_goals.toLowerCase().includes('learn')) metrics.growth_mindset_score += 20;
    if (answers.growth_goals.toLowerCase().includes('improve')) metrics.growth_mindset_score += 20;
    if (answers.growth_goals.toLowerCase().includes('develop')) metrics.growth_mindset_score += 10;
  }

  // Resilience Score (based on strategies and practices)
  if (answers.neuroscience_practices?.length > 50) metrics.resilience_score += 40;
  ...
  Object.keys(metrics).forEach(key => {
    metrics[key as keyof GrowthMetrics] = Math.min(100, Math.max(0, metrics[key as keyof GrowthMetrics]));
  });
...
async function trackGrowthTrajectory(
  ...
  try {
    ...
    if (previousData && previousData.length > 1) {
      ...
      const trajectory = {
        user_id: userId,
        preparedness_change: currentMetrics.preparedness_score - (previous.preparedness_score || 0),
        self_awareness_change: currentMetrics.self_awareness_level - (previous.self_awareness_level || 0),
        role_clarity_change: currentMetrics.role_clarity_score - (previous.role_clarity_score || 0),
        ethical_awareness_change: currentMetrics.ethical_awareness_score - (previous.ethical_awareness_score || 0),
        growth_mindset_change: currentMetrics.growth_mindset_score - (previous.growth_mindset_score || 0),
        resilience_change: currentMetrics.resilience_score - (previous.resilience_score || 0),
        overall_change: currentMetrics.overall_progress - (previous.overall_progress || 0),
...
export async function getUserGrowthInsights(userId: string): Promise<{
  ...
  try {
    ...
    return {
      error: error instanceof Error ? error.message : 'Failed to retrieve growth insights'
    };
  }
}
...
export function generateRecommendations(metrics: GrowthMetrics): string[] {
  ...
  if (metrics.preparedness_score < 70) {
    recommendations.push('Consider spending more time reviewing materials before assignments');
  }

  if (metrics.self_awareness_level < 70) {
    recommendations.push('Practice daily self-check-ins to improve emotional awareness');
  }

  if (metrics.role_clarity_score < 70) {
    recommendations.push('Review professional boundaries and role-space guidelines');
  }

  if (metrics.ethical_awareness_score < 70) {
    recommendations.push('Engage with ethics case studies and reflection exercises');
  }
  ...
  if (metrics.resilience_score < 70) {
    recommendations.push('Develop a toolkit of coping strategies and self-care practices');
  }

  if (metrics.overall_progress > 80) {
    recommendations.push('You\'re doing great! Consider mentoring others or taking on more challenging assignments');
  }
