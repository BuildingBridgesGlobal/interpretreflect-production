...
try {
  let content = fs.readFileSync('src/components/DailyBurnoutGaugeAccessible.tsx', 'utf8');
...
try {
  let content = fs.readFileSync('src/components/EmotionMappingAccessible.tsx', 'utf8');
...
try {
  let content = fs.readFileSync('src/components/GrowthInsightsDashboard.tsx', 'utf8');
  content = content.replace(/Award,?\s*\n\s*TrendingUp/g, 'Award,\n  TrendingUp');
...
try {
  let content = fs.readFileSync('src/components/TemperatureExploration.tsx', 'utf8');
  content = content.replace(/useState<(\w+)>\((\{[^}]+)\);/g, function(match, type, obj) {
    if (!obj.includes(',')) {
      return match.replace(obj, obj.replace(/([a-zA-Z0-9_]+):\s*([^,}]+)/g, '$1: $2,').replace(/,\s*}/, ' }'));
    }
    return match;
  });
...
filesToFixKeyof.forEach(filePath => {
  try {
    ...
    // Fix keyof syntax
    content = content.replace(/keyof\s+typeof\s+(\w+)\s*\)/g, 'keyof typeof $1)');
...
try {
  let content = fs.readFileSync('src/components/MentoringReflectionAccessible.tsx', 'utf8');
