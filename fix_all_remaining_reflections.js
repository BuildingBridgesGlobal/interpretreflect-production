...
componentsToFix.forEach(filename => {
  ...
  if (!content.includes('directSupabaseApi') && !content.includes('directInsertReflection')) {
  ...
  if (content.includes('handleSubmit') || content.includes('handleSave')) {
    ...
    const functionMatch = content.match(/(handle(?:Submit|Save))[^{]*{[\s\S]*?^  }/gm);
