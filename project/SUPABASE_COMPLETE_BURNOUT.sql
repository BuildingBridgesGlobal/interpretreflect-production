-- =====================================================
-- COMPLETE BURNOUT RISK MONITOR SETUP FOR SUPABASE
-- =====================================================

-- STEP 1: Enable required extension (MUST RUN FIRST)
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Wait a moment for extension to be available, then continue...
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_wellness_user ON public.wellness_metrics(user_hash);
CREATE INDEX IF NOT EXISTS idx_wellness_week ON public.wellness_metrics(week_of DESC);

-- Create anonymized_reflections table
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_anon_user_hash ON public.anonymized_reflections(user_hash);
CREATE INDEX IF NOT EXISTS idx_anon_created ON public.anonymized_reflections(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_pattern_user ON public.pattern_insights(user_hash);
CREATE INDEX IF NOT EXISTS idx_pattern_month ON public.pattern_insights(month_of DESC);
...
CREATE INDEX IF NOT EXISTS idx_audit_action ON public.privacy_audit_logs(action_type);
CREATE INDEX IF NOT EXISTS idx_audit_time ON public.privacy_audit_logs(occurred_at DESC);

-- =====================================================
-- STEP 3: CREATE FUNCTIONS (After pgcrypto is enabled)
-- =====================================================

-- Drop existing functions if they exist (to avoid conflicts)
DROP FUNCTION IF EXISTS public.create_user_hash(UUID, TEXT);
DROP FUNCTION IF EXISTS public.predict_burnout_risk_zkwv(TEXT);

-- Create user hash function using pgcrypto's digest
CREATE OR REPLACE FUNCTION public.create_user_hash(user_id UUID, salt TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
SET search_path = ''
AS $$
  SELECT encode(digest((user_id::text || salt)::text, 'sha256'), 'hex');
$$;

-- Create burnout prediction function
CREATE OR REPLACE FUNCTION public.predict_burnout_risk_zkwv(p_user_hash TEXT)
RETURNS JSON
LANGUAGE plpgsql
SET search_path = ''
AS $$
DECLARE
  ...
  v_trend TEXT;
BEGIN
  -- Get metrics from wellness_metrics table
  SELECT
    AVG(stress_level),
    AVG(energy_level),
    AVG(burnout_score),
...
DROP TRIGGER IF EXISTS set_timestamp ON public.wellness_metrics;
...
-- Insert test wellness metrics
WITH user_data AS (
  SELECT DISTINCT user_id FROM public.reflection_entries LIMIT 1
)
INSERT INTO public.wellness_metrics (
  user_hash, stress_level, energy_level, burnout_score, confidence_score,
  high_stress_pattern, recovery_needed, week_of
)
SELECT
  encode(digest((user_id::text || 'interpretreflect-zkwv-2025')::text, 'sha256'), 'hex') as user_hash,
  CASE week_num
    WHEN 0 THEN 5.5
    WHEN 1 THEN 6.8
    WHEN 2 THEN 7.5
  ...
  CASE week_num WHEN 0 THEN true WHEN 1 THEN true ELSE false END as high_stress_pattern,
  CASE week_num WHEN 0 THEN true ELSE false END as recovery_needed,
...
-- Insert test anonymized reflections
WITH user_data AS (
  SELECT DISTINCT user_id FROM public.reflection_entries LIMIT 1
)
INSERT INTO public.anonymized_reflections (
...
-- Check extension
SELECT 'pgcrypto extension:' as check,
...
-- Check data
SELECT 'Wellness metrics:' as check, COUNT(*) as count FROM public.wellness_metrics;
SELECT 'Anonymized reflections:' as check, COUNT(*) as count FROM public.anonymized_reflections;
...
SELECT
  '🔥 Burnout Risk Assessment' as check,
  public.predict_burnout_risk_zkwv(
    encode(digest((user_id::text || 'interpretreflect-zkwv-2025')::text, 'sha256'), 'hex')
  ) as assessment
FROM user_data;

-- Final success message
SELECT '✅ Burnout Risk Monitor is now fully operational!' as message;