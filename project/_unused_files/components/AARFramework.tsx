...
interface AARFrameworkProps {
	onClose: () => void;
	onComplete?: () => void;
}

export const AARFramework: React.FC<AARFrameworkProps> = ({
	onClose,
	onComplete,
}) => {
	const framework = deepDiveFrameworks.aar;
	...
	const sectionIcons = [
		<ClipboardList className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Activity className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Search className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Users className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Lightbulb className="w-5 h-5" style={{ color: "#6B8B60" }} />,
	];
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const { error } = await supabase.from("reflections").insert({
				user_id: user.id,
				reflection_type: "after_action_review",
				content: formData,
