...
export const AssignmentReset: React.FC<AssignmentResetProps> = ({
	onClose,
	onComplete,
}) => {
					...
					<div className="p-6">
						<div className="flex justify-between items-start mb-6">
							<div>
								...
								<p className="text-gray-600">Clear between interpretations</p>
							</div>
							<button
								onClick={onClose}
	...
	if (phase === "practice") {
		...
		const getPracticeContent = () => {
			switch (selectedDuration) {
				case "10s":
					return {
						title: "Quick Reset",
						guidance: ["Out with the old.", "In with the new."],
					};
				case "30s":
					return {
						title: "Resetting",
		...
		const currentGuidance = Math.floor(
			(timeElapsed / getDurationSeconds()) * content.guidance.length,
		);
