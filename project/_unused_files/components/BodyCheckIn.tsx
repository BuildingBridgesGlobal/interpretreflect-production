...
export const BodyCheckIn: React.FC<BodyCheckInProps> = ({
	...
	const [selectedDuration, setSelectedDuration] =
		useState<PracticeDuration>(duration);
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const { error } = await supabase.from("body_checkins").insert({
				...
				overall_feeling: Math.round(
					(tensionLevel + energyLevel + moodLevel) / 3,
				),
					...
					<div className="p-8">
						<div className="flex justify-between items-start mb-6">
							<div>
								...
								<p className="text-gray-600">Notice how your body feels</p>
							</div>
							<button
								onClick={onClose}
						...
						<div className="mb-6 p-4 bg-gray-50 rounded-xl">
							<p className="text-sm text-gray-700">
								<strong>Tip:</strong> Your body holds the day's work. Checking
								in helps you release it.
							</p>
						</div>
				...
				<div className="rounded-3xl max-w-lg w-full bg-white">
					<div className="p-8">
						...
						<div className="text-center space-y-4">
							...
							{content.ending && (
								<p className="text-gray-600 font-medium">{content.ending}</p>
							)}
						</div>
			...
			<div className="rounded-3xl max-w-lg w-full bg-white">
				<div className="p-8">
					...
					<div className="mb-8">
						...
						<div className="space-y-2">
							{["All good", "Shoulders/neck", "Back", "Other areas"].map(
								(option) => (
									<label key={option} className="flex items-center">
										<input
											type="radio"
