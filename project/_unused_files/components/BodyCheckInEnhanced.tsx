...
export const BodyCheckIn: React.FC<BodyCheckInProps> = ({
	onClose,
	onComplete,
}) => {
	...
	const [selectedDuration, setSelectedDuration] =
		useState<PracticeDuration>("1m");
	const [selectedApproach, setSelectedApproach] =
		useState<ApproachType>("scan-release");
	const [currentArea, setCurrentArea] = useState<BodyArea>("head-jaw");
	const [timeElapsed, setTimeElapsed] = useState(0);
	const [isActive, setIsActive] = useState(false);
	const [justFinishedAssignment, setJustFinishedAssignment] = useState<
		boolean | null
	>(null);
				...
				setTimeElapsed((prev) => {
					...
					if (selectedDuration === "1m") {
						...
						if (
							...
							areas[currentIndex] !== currentArea
						) {
					...
					<div className="p-8">
						<div className="flex justify-between items-start mb-6">
							<div>
								...
								<p className="text-gray-600">Release what you're holding</p>
							</div>
							<button
								onClick={onClose}
						...
						<div className="mb-6 p-4 bg-gray-50 rounded-xl">
							<p className="text-sm text-gray-700">
								<strong>Common holding patterns:</strong> Interpreters often
								carry tension in jaw (concentration), shoulders (stress), and
								hands (signing/writing).
							</p>
						</div>
				...
				case "2m":
					return {
						...
						deepGuidance: [
							...
							"What's tight (information)",
							"What's easy (resource)",
				...
				case "3m":
					return {
						...
						deepGuidance: [
							...
							"Check:",
							"Repetitive strain areas",
							"Emotional holding spots",
							"Fatigue indicators",
							"Adjust what needs adjusting.",
							"Document patterns for later.",
						],
					};
				default:
					return {
								...
								<button
									...
									onClick={() => {
										if (area.value === "clear") {
											...
											setTensionMap([
												...tensionMap.filter((t) => t !== "clear"),
												area.value as TensionArea,
											]);
										}
									}}
								...
								<div className="space-y-3">
									{tensionMap.includes("head-jaw") && (
										<p className="text-sm text-gray-500">
											If you noticed{" "}
											<span className="font-medium">jaw tension</span> → You
											might be concentrating really hard today
										</p>
									)}
									{tensionMap.includes("shoulders-neck") && (
										<p className="text-sm text-gray-500">
											If you noticed{" "}
											<span className="font-medium">shoulder tightness</span> →
											You're probably carrying some stress
										</p>
									)}
									{tensionMap.includes("back") && (
										<p className="text-sm text-gray-500">
											If you noticed{" "}
											<span className="font-medium">back tension</span> → Your
											posture might need some love
										</p>
									)}
									{tensionMap.includes("hands-arms") && (
										<p className="text-sm text-gray-500">
											If you noticed{" "}
											<span className="font-medium">hand strain</span> → Your
											hands have been working hard
										</p>
									)}
								</div>
