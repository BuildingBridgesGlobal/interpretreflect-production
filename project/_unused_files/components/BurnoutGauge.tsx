...
export const BurnoutGauge: React.FC<BurnoutGaugeProps> = ({
	...
	const getGaugeMessage = (score: number) => {
		if (score >= 8)
			return {
				text: "High energy - You're in a good place!",
				icon: <Heart className="w-5 h-5" />,
			};
		if (score >= 6)
			return {
				text: "Moderate energy - Monitor your wellbeing",
				icon: <Battery className="w-5 h-5" />,
			};
		if (score >= 4)
			return {
				text: "Low energy - Consider a wellness check-in",
				icon: <BatteryLow className="w-5 h-5" />,
			};
		return {
			text: "Very low energy - Prioritize self-care today",
	...
	const gaugeColor = lastScore
		? getGaugeColor(lastScore)
					...
					{hasCompletedToday ? (
						<>
							...
							<div
								...
								aria-label={`Energy level: ${lastScore} out of 10`}
							>
							...
							{gaugeInfo && (
								<div
									className={`flex items-center space-x-2 p-3 rounded-md ${gaugeColor}`}
								>
									{gaugeInfo.icon}
