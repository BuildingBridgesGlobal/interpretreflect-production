...
export const CodeSwitchReset: React.FC<CodeSwitchResetProps> = ({
	onClose,
	onComplete,
}) => {
	// Core states
	...
	const [selectedMethod, setSelectedMethod] =
		useState<ResetMethod>("combination");
	const [assignmentType, setAssignmentType] =
		useState<AssignmentType>("between");
	...
	const handleSubmit = () => {
		...
		if (trackOverTime) {
			const sessions = JSON.parse(
				localStorage.getItem("codeSwitchSessions") || "[]",
			);
			sessions.push(data);
		...
		if (assignmentResidue && assignmentResidue.length > 50) {
			console.warn(
				"Assignment residue may need supervision attention:",
				assignmentResidue,
			);
		}
					...
					<div className="p-8">
						...
						<div className="flex justify-between items-start mb-8">
							<div>
								...
								<p
									className={`text-lg ${highContrastMode ? "text-black" : "text-gray-600"}`}
								>
												...
												<div className="flex-1">
													<h3
														className={`font-semibold mb-2 ${
															selectedMethod === key && highContrastMode
																? "text-white"
																: ""
														}`}
													>
						...
						>
							...
							<p
								className={`text-sm ${highContrastMode ? "text-black" : "text-gray-700"}`}
							>
								Your brain needs to clear residual activation patterns between
								languages. This practice helps reset your linguistic channels,
								reducing interference and improving accuracy for your next
								interpretation.
							</p>
						</div>
						...
						>
							<h3
								className={`font-medium mb-2 ${highContrastMode ? "text-black" : "text-amber-900"}`}
							>
								Comfort Reminders:
							</h3>
							<ul
			...
			<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
				...
				>
					<div className="p-8">
						...
						>
							<p
								className={`text-sm mb-4 ${highContrastMode ? "text-black" : "text-gray-600"}`}
							>
								Your Reset Journey:
							</p>
							<div className="flex items-center justify-center gap-2 text-sm">
								...
								<span
									className={`px-3 py-1 rounded ${
										timeElapsed >= getDurationSeconds() * 0.25 &&
										timeElapsed < getDurationSeconds() * 0.5
											? highContrastMode
												? "bg-black text-white"
												: "bg-blue-600 text-white"
											: highContrastMode
						...
						<div className="mb-8">
							<div className="flex justify-between items-center mb-2">
								<span
									className={`font-medium ${highContrastMode ? "text-black" : "text-gray-700"}`}
								>
									Progress:
								</span>
								<span
									className={`text-lg font-bold ${highContrastMode ? "text-black" : "text-gray-900"}`}
								>
		...
		<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
			...
			>
				<div className="p-8">
					...
					<p
						className={`mb-8 ${highContrastMode ? "text-black" : "text-gray-600"}`}
					>
					...
					<div className="mb-6">
						<h3
							className={`font-medium mb-4 ${highContrastMode ? "text-black" : "text-gray-700"}`}
						>
							...
							{[
								{
									...
									value: mentalClarity,
									setter: setMentalClarity,
								},
								{
									label: "Physical readiness",
									value: physicalReadiness,
									setter: setPhysicalReadiness,
								},
								{
									label: "Emotional neutrality",
									value: emotionalNeutrality,
									setter: setEmotionalNeutrality,
								},
								{
									label: "Cultural positioning",
									value: culturalPositioning,
									setter: setCulturalPositioning,
								},
							].map((scale) => (
								<div key={scale.label}>
					...
					>
						...
						>
							Are you still holding anything from the previous interpretation
					...
					<div className="mb-8">
						<h3
							className={`font-medium mb-3 ${highContrastMode ? "text-black" : "text-gray-700"}`}
						>
							Your Preferences:
						</h3>
						<div className="space-y-2">
