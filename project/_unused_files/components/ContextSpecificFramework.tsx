...
import { saveContextReflection } from "../services/contextReflectionService";

interface ContextSpecificFrameworkProps {
	onClose: () => void;
	onComplete?: () => void;
}

export const ContextSpecificFramework: React.FC<
	ContextSpecificFrameworkProps
> = ({ onClose, onComplete }) => {
	const framework = deepDiveFrameworks.contextSpecific;
	...
	const sectionIcons = [
		<Stethoscope className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Scale className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<GraduationCap className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Brain className="w-5 h-5" style={{ color: "#6B8B60" }} />,
	...
	const handleSubmit = async () => {
		...
		try {
			...
			// Use the enhanced context reflection service
			const result = await saveContextReflection(
				user.id,
				"context_specific",
				formData,
			);
			...
			console.log(
				"Context reflection saved with insights:",
								...
								>
									<div
										style={{
											...
											borderRadius: "50%",
											backgroundColor:
												index <= currentSection ? "#6B8B60" : "#E5E5E5",
											display: "flex",
											alignItems: "center",
											justifyContent: "center",
							...
							<div key={question.id}>
								...
								<textarea
									...
									style={{
										...
										padding: "12px",
										borderRadius: "8px",
										border: validationErrors.has(question.id)
											? "2px solid #EF4444"
											: "1px solid #E5E5E5",
										fontSize: "14px",
										resize: "vertical",
				...
				>
					...
					{currentSection === framework.sections.length - 1 ? (
						...
						<button
							...
							onMouseEnter={(e) =>
								(e.currentTarget.style.backgroundColor = "#5F7F55")
							}
							onMouseLeave={(e) =>
								(e.currentTarget.style.backgroundColor = "#6B8B60")
							}
						>
							Next
