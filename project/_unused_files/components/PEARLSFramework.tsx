...
interface PEARLSFrameworkProps {
	onClose: () => void;
	onComplete?: () => void;
}

export const PEARLSFramework: React.FC<PEARLSFrameworkProps> = ({
	onClose,
	onComplete,
}) => {
	const framework = deepDiveFrameworks.pearls;
	...
	const sectionIcons = [
		...
		<Heart className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Shield className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<CheckCircle className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<LifeBuoy className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Activity className="w-5 h-5" style={{ color: "#6B8B60" }} />,
	];
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const { error } = await supabase.from("reflections").insert({
				user_id: user.id,
				reflection_type: "pearls_framework",
				content: formData,
