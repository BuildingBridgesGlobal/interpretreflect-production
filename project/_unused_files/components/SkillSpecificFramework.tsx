...
interface SkillSpecificFrameworkProps {
	onClose: () => void;
	onComplete?: () => void;
}

export const SkillSpecificFramework: React.FC<SkillSpecificFrameworkProps> = ({
	onClose,
	onComplete,
}) => {
	const framework = deepDiveFrameworks.skillSpecific;
	...
	const sectionIcons = [
		<Target className="w-5 h-5" style={{ color: "#6B8B60" }} />,
		<Handshake className="w-5 h-5" style={{ color: "#6B8B60" }} />,
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const { error } = await supabase.from("reflections").insert({
				user_id: user.id,
				reflection_type: "skill_specific",
				content: formData,
