...
export const TeamingPrepReflection: React.FC<TeamingPrepReflectionProps> = ({
	...
	const handleFieldChange = (
		fieldId: string,
		value: string | number | string[] | "Done!",
	) => {
		setFormData((prev) => ({
			...prev,
			[fieldId]: value,
	...
	const validateSection = (sectionIndex: number): boolean => {
		...
		switch (section.id) {
			...
			case "signal_plan":
				...
				if (
					formData.signal_type === "Other" &&
					!formData.signal_custom?.trim()
				) {
									...
									<>
										...
										<div className="flex items-center space-x-4">
											<input
												...
												step={field.step}
												value={
													formData[field.id as keyof TeamingPrepData] as number
												}
												onChange={(e) =>
								...
								{field.type === "text" && (
									<>
										...
										<input
											type="text"
											value={
												formData[field.id as keyof TeamingPrepData] as string
											}
											onChange={(e) =>
								...
								{field.type === "select" && (
									<>
										...
										<select
											...
											onChange={(e) =>
												handleFieldChange("signal_type", e.target.value)
											}
								...
								{field.type === "text" && formData.signal_type === "Other" && (
									<>
										...
										<input
											...
											onChange={(e) =>
												handleFieldChange("signal_custom", e.target.value)
											}
											maxLength={field.maxLength}
											className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-sage-500"
											style={{
												borderColor: errors.signal_custom
													? "#ef4444"
													: "#E8E5E0",
											}}
										/>
								...
								<div className="grid grid-cols-2 gap-3">
									{section.checklist.options.map((option: string) => (
										<label
											...
											style={{
												borderColor: formData.stressors.includes(option)
													? "#6B8B60"
													: "#E8E5E0",
												backgroundColor: formData.stressors.includes(option)
								...
								<div className="space-y-2">
									{section.fallbackPlan.options.map((option: string) => (
										<label
											...
											style={{
												borderColor:
													formData.plan_if_sideways === option
														? "#6B8B60"
														: "#E8E5E0",
												backgroundColor:
													formData.plan_if_sideways === option
										...
										>
											<input
												...
												onChange={(e) =>
													handleFieldChange("plan_if_sideways", e.target.value)
												}
												style={{ accentColor: "#6B8B60" }}
											/>
											<span className="text-sm" style={{ color: "#1A1A1A" }}>
							...
							{section.confirmField && (
								<div className="mt-4">
									<label
										...
										style={{
											borderColor:
												formData.viz_done === "Done!" ? "#6B8B60" : "#E8E5E0",
											backgroundColor:
												formData.viz_done === "Done!"
													? "#FFFFFF"
													: "transparent",
										}}
									>
				...
				return (
					<div className="space-y-6">
						...
						<textarea
							...
							onChange={(e) =>
								handleFieldChange("micro_intention", e.target.value)
							}
						...
						<div className="flex space-x-2">
							{TEAMING_PREP.sections.map((_, index) => (
								<div
									...
									style={{
										backgroundColor:
											index <= currentSection ? "#6B8B60" : "#E8E5E0",
										minWidth: "40px",
									}}
								/>
							))}
