...
const TeamingReflection: React.FC<TeamingReflectionProps> = ({
	onComplete,
	onClose,
}) => {
	const { user } = useAuth();
	...
	const handleComplete = async () => {
		...
		const loadBalanceEffective =
			plannedLoad === actualLoad || actualLoad === "50-50";

		// Calculate error count from errorsCaught string
		const errorCount =
			errorsCaught === "none"
				? 0
				: errorsCaught === "1-2-minor"
					? 2
					: errorsCaught === "3-5-minor"
						? 4
						: errorsCaught === "1-2-major"
							? 2
							: errorsCaught === "several-major"
								? 5
								: 0;

		// Calculate recovery rate based on recovery success
		const recoveryRate =
			recoverySuccess === "smooth"
				? 100
				: recoverySuccess === "adequate"
					? 75
					: recoverySuccess === "struggled"
						? 40
						: recoverySuccess === "failed"
							? 0
							: 50;

		// Calculate trust score based on partner seemed and comfortable items
		const trustScore =
			partnerSeemed === "supportive"
				? 10
				: partnerSeemed === "professional"
					? 8
					: partnerSeemed === "critical"
						? 5
						: partnerSeemed === "judgmental"
							? 3
							: partnerSeemed === "unavailable"
								? 1
								: 7;
		...
		const actionableItems = [partnerIdea, selfWorkOn, selfPractice].filter(
			Boolean,
		).length;
						...
						<div className="flex items-center space-x-3">
							...
							<div>
								...
								<p className="text-sm text-gray-600 mt-1">
									5-minute team debrief protocol
								</p>
							</div>
						</div>
						<button
						...
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									45 seconds • Based on Collective Efficacy research (Bandura,
									1997)
								</p>
							</div>
							...
							<div className="space-y-4">
								...
								<div>
									...
									<div className="space-y-2">
										{[
											...
											{
												value: "emoji",
												label: "Text emoji to partner (if remote)",
												icon: "📱",
											},
											{
												value: "internal",
								...
								<div className="bg-yellow-100 p-4 rounded-lg">
									<p className="text-sm text-yellow-800">
										<strong>Research insight:</strong> Teams that celebrate wins
										show 34% better performance in future sessions (Gully et
										al., 2002)
									</p>
								</div>
							</div>
					...
					{currentStep === 2 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									90 seconds • Based on Team Coordination Dynamics (Rico et al.,
									2008)
								</p>
							</div>
							...
							<div className="space-y-4">
								...
								<div>
									...
									<div className="space-y-2">
										{[
											...
											{
												value: "timely",
												label: "Support came right when requested",
											},
											{ value: "delayed", label: "Some delays but recovered" },
											{
												value: "failed",
												label: "Support system didn't work today",
											},
										].map((option) => (
											<label
												key={option.value}
								...
								<div className="bg-blue-100 p-4 rounded-lg">
									<p className="text-sm text-blue-800">
										<strong>Research insight:</strong> Teams analyzing
										coordination improve by 41% within 5 sessions (Kozlowski &
										Ilgen, 2006)
									</p>
								</div>
							</div>
					...
					{currentStep === 3 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									60 seconds • Based on Distributed Cognition Theory (Hutchins,
									1995)
								</p>
							</div>
							...
							<div className="space-y-4">
									...
									<div>
										...
										<div className="space-y-2">
											{[
												{
													value: "complexity",
													label: "Content complexity surprised us",
												},
												{
													value: "energy",
													label: "One partner's energy dropped",
												},
												{
													value: "technical",
													label: "Technical terminology imbalance",
												},
												{
													value: "speed",
													label: "Speed overwhelmed one partner",
												},
												{
													value: "emotional",
													label: "Emotional content impact",
												},
											].map((option) => (
												<label
													key={option.value}
								...
								<div className="bg-purple-100 p-4 rounded-lg">
									<p className="text-sm text-purple-800">
										<strong>Research insight:</strong> Teams that adjust load
										distribution based on reality show 55% less burnout (Seeber,
										2011)
									</p>
								</div>
							</div>
					...
					{currentStep === 4 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									60 seconds • Based on High Reliability Teams research (Weick &
									Sutcliffe, 2001)
								</p>
							</div>
							...
							<div className="space-y-4">
										...
										{[
											{
												value: "smooth",
												label:
													"Smooth recoveries - audience likely didn't notice",
											},
											{
												value: "adequate",
												label: "Adequate - minor disruption",
											},
											{
												value: "struggled",
												label: "Struggled - obvious difficulties",
											},
											{
												value: "failed",
												label: "Failed to recover - errors remained",
											},
										].map((option) => (
											<label
												key={option.value}
								...
								<div className="bg-orange-100 p-4 rounded-lg">
									<p className="text-sm text-orange-800">
										<strong>Research insight:</strong> Teams that debrief errors
										reduce repeat mistakes by 67% (Gersick & Hackman, 1990)
									</p>
								</div>
							</div>
						</div>
					...
					{currentStep === 5 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									60 seconds • Based on Team Psychological Safety (Edmondson,
									1999)
								</p>
							</div>
							...
							<div className="space-y-4">
								...
								<div className="bg-red-100 p-4 rounded-lg">
									<p className="text-sm text-red-800">
										<strong>Research insight:</strong> Teams with high
										psychological safety perform 47% better (Edmondson, 2012)
									</p>
								</div>
							</div>
						</div>
					...
					{currentStep === 6 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									45 seconds • Based on Effective Feedback research (Kluger &
									DeNisi, 1996)
								</p>
							</div>
							...
							<div className="space-y-6">
								...
								<div className="bg-green-100 p-4 rounded-lg">
									<p className="text-sm text-green-800">
										<strong>Research insight:</strong> Specific behavioral
										feedback improves performance 39% more than general feedback
					...
					{currentStep === 7 && (
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4 italic">
									30 seconds • Based on Team Learning Theory (Argote et al.,
									2001)
								</p>
							</div>
							...
							<div className="space-y-4">
								...
								<div className="bg-gray-50 p-4 rounded-lg">
									...
									<ul className="space-y-2 text-sm text-gray-700">
										<li>
											• <strong>Team debriefs</strong> improve next performance
											by 25% (Tannenbaum & Cerasoli, 2013)
										</li>
										<li>
											• <strong>Trust assessment</strong> predicts team success
											with 82% accuracy (De Jong et al., 2016)
										</li>
										<li>
											• <strong>Specific feedback</strong> accelerates skill
											development by 40% (Villado & Arthur, 2013)
										</li>
										<li>
											• <strong>Pattern recognition</strong> reduces team errors
											by 58% over time (Schippers et al., 2013)
										</li>
									</ul>
								</div>
							</div>
						...
						<div className="space-y-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4">
									Your team insights have been saved!
								</p>
							</div>
							...
							<div className="bg-gradient-to-r from-purple-500 to-blue-500 p-6 rounded-xl text-white">
								<p className="text-lg font-semibold text-center">
									Team reflection complete! Your partnership insights have been
									captured.
								</p>
							</div>
