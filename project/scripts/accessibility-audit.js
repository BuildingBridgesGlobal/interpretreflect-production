...
async function runAccessibilityAudit() {
	try {
		...
		const checks = [
			...
			},
			{
				...
				test: () => {
					...
					const labelCount =
						countOccurrences(html, "<label") +
						countOccurrences(html, "aria-label");
			...
			},
			{
				...
				test: () =>
					html.includes("Skip to main content") || html.includes("skip-link"),
				level: "A",
				category: "Navigation",
				wcag: "2.4.1",
			},
			...
			// ARIA Implementation
			{
				...
				test: () => {
					const hasMain =
						html.includes('role="main"') || html.includes("<main");
					const hasNav =
						html.includes('role="navigation"') || html.includes("<nav");
					const hasHeader =
						html.includes('role="banner"') || html.includes("<header");
			...
			},
			{
				...
				test: () =>
					html.includes("aria-label") || html.includes("aria-labelledby"),
				level: "Best Practice",
				category: "ARIA",
				wcag: "N/A",
			},
			...
			// Keyboard Navigation
			{
				...
				test: () =>
					!html.includes('tabindex="-1"') || html.includes('tabindex="0"'),
				level: "A",
				category: "Keyboard",
				wcag: "2.1.1",
			},
		...
		console.log(
			`${colors.bright}Running WCAG 2.1 Compliance Checks${colors.reset}`,
		);
		...
		Object.keys(categories).forEach((category) => {
			...
			categories[category].forEach((check) => {
				...
				if (passed) {
					console.log(
						`${colors.green}✅ ${check.name} (WCAG ${check.wcag})${colors.reset}`,
					);
					passCount++;
				} else if (check.warning) {
					console.log(
						`${colors.yellow}⚠️  ${check.name} (WCAG ${check.wcag}) - Manual check needed${colors.reset}`,
					);
					warnCount++;
					issues.push({
						level: check.level,
					...
					console.log(
						`${colors.red}❌ ${check.name} (WCAG ${check.wcag})${colors.reset}`,
					);
					failCount++;
					issues.push({
						level: check.level,
		...
		if (issues.length > 0) {
			...
			issues
				...
				.forEach((issue, idx) => {
					console.log(
						`${idx + 1}. ${issue.issue} (WCAG ${issue.wcag} - Level ${issue.level})`,
					);
				});
		}
		...
		console.error(
			`${colors.red}Error running accessibility audit: ${error.message}${colors.reset}`,
		);
		console.error(
			`${colors.yellow}Make sure the dev server is running at ${URL}${colors.reset}`,
		);
		process.exit(1);
	}
}
