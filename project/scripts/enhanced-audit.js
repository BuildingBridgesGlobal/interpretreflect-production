			...
			client
				...
				.on("error", (err) => {
					if (attemptNum < retries) {
						console.log(
							`${colors.yellow}Retry ${attemptNum + 1}/${retries}...${colors.reset}`,
						);
...
async function runEnhancedAudit() {
	try {
		console.log(
			`${colors.blue}Fetching page content (waiting for React render)...${colors.reset}\n`,
		);
		...
		const checks = [
			...
			},
			{
				...
				test: () =>
					html.includes('role="contentinfo"') || html.includes("<footer"),
				category: "Accessibility",
				wcag: "1.3.1",
				level: "A",
			},
		...
		console.log(
			`${colors.blue}📄 HTML report saved to: ${htmlPath}${colors.reset}`,
		);
...
`;
		...
		console.log(
			`${colors.blue}📄 Text summary saved to: ${textPath}${colors.reset}`,
		);
		...
		console.log(
			`${colors.cyan}📁 All reports saved to: ${auditDir}${colors.reset}`,
		);
	} catch (error) {
		console.error(
			`${colors.red}Error running audit: ${error.message}${colors.reset}`,
		);
		console.error(
