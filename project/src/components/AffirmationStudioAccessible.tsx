...
const affirmationCategories: AffirmationCategory[] = [
	{
		...
		reflection:
			"How can I remember my inherent worth today, separate from my work?",
		actionableTips: [
			{
				challenge:
					"Write down three things you value about yourself that have nothing to do with work",
			...
			{
				challenge:
					"Set aside 10 minutes today for something that brings you joy, regardless of its productivity value",
				tip: "Listen to music, doodle, take a walk, or simply sit quietly. Joy for its own sake reinforces your inherent worth.",
			},
			{
				challenge:
					"Practice receiving a compliment without deflecting or minimizing it",
	...
	},
	{
		...
		description:
			"Celebrating your skills, growth, and professional contributions",
		icon: Award,
		color: "#8B7355",
		bgColor: "#FFF9F0",
		borderColor: "#C4A57B",
		...
		reflection:
			"What professional skill or achievement am I most proud of today?",
		actionableTips: [
			{
				challenge:
					"Share one piece of professional wisdom with a colleague or peer today",
				tip: "Your experience has value. Sharing what you've learned helps others and reinforces your own expertise.",
			},
			{
				challenge:
					"Document one successful problem-solving moment from your recent work",
				tip: "Write a brief note about how you handled a challenge. This creates a personal library of your competence.",
			},
			{
				challenge:
					"Ask a thoughtful question in your next meeting or interaction",
	...
	},
	{
		...
		description:
			"Honoring your ability to weather storms and bounce back from difficulty",
		icon: Zap,
		color: "#7A8B9B",
		bgColor: "#F0F3F7",
		borderColor: "#9BADC4",
		...
		reflection:
			"What challenge have I overcome recently that shows my resilience?",
		actionableTips: [
			{
				challenge:
					"Identify one coping strategy that worked for you recently and use it proactively today",
				tip: "Whether it's deep breathing, taking breaks, or reaching out to someone, use what works before stress builds up.",
			},
			{
				challenge:
					"Acknowledge one difficult emotion without trying to fix or change it",
			...
			{
				challenge:
					"Do one small thing today that your future self will thank you for",
	...
	},
	{
		...
		description:
			"Celebrating your commitment to personal and professional development",
		icon: TrendingUp,
		color: "#6B8B90",
		bgColor: "#EFF5F6",
		borderColor: "#89B4BB",
		...
		actionableTips: [
			{
				challenge:
					"Try one new approach or technique in your work today, no matter how small",
				tip: "Growth happens through experimentation. Even tiny changes keep your mind flexible and curious.",
			},
			{
				challenge:
					"Write down one question you're curious about related to your field",
				tip: "Curiosity is the engine of growth. Identifying what you want to learn gives direction to your development.",
			},
			{
				challenge:
					"Celebrate one learning moment from today, especially if it came from a mistake",
	...
	},
	{
		...
		description:
			"Connecting with deeper meaning and purpose in your work and life",
		icon: Target,
		color: "#8B6B8B",
		bgColor: "#F7F0F7",
		borderColor: "#B499B4",
		...
		reflection:
			"How did my work make a positive difference today, even in small ways?",
		actionableTips: [
			{
				challenge:
					"Notice and appreciate one small positive impact you made today",
	...
	},
	{
		...
		description:
			"Affirming your right to protect your energy, time, and wellbeing",
		icon: Shield,
		color: "#8B8B6B",
		bgColor: "#F7F7F0",
		borderColor: "#B4B499",
		...
		reflection:
			"What boundary do I need to set or maintain to protect my wellbeing?",
		actionableTips: [
			{
...
const AffirmationStudioAccessible: React.FC = () => {
	...
	const generateDailyAffirmation = () => {
		const categoryIndex = Math.floor(
			Math.random() * affirmationCategories.length,
		);
		const affirmationIndex = Math.floor(Math.random() * 5);
		const daily = {
			category: affirmationCategories[categoryIndex].id,
	...
	const handleSelectCategory = (category: AffirmationCategory) => {
		...
		const updated = [
			category.id,
			...recentlyUsed.filter((id) => id !== category.id),
		].slice(0, 3);
	...
	const getDailyAffirmationText = () => {
		...
		const category = affirmationCategories.find(
			(c) => c.id === dailyAffirmation.category,
		);
		...
		>
			...
			<header className="mb-8">
				...
				<p className="text-lg" style={{ color: "#4A5568" }}>
					...
					Use before, after, or anytime during your day.
				</p>
			</header>
						...
						<div className="p-6">
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm" style={{ color: "#718096" }}>
											Affirmation {currentAffirmationIndex + 1} of{" "}
											{selectedCategory.affirmations.length}
										</p>
									</div>
								</div>
								<button
							...
							{!showReflection ? (
								<>
									...
									>
										<blockquote
											className="text-xl text-center italic font-medium"
											style={{ color: selectedCategory.color }}
										>
									...
									<div className="flex gap-3 mb-4">
										<button
											...
											style={{
												backgroundColor: favorites.has(
													`${selectedCategory.id}-${currentAffirmationIndex}`,
												)
													? "#FFF9F0"
													: "#F7FAFC",
												color: favorites.has(
									...
									<div className="flex items-center justify-between">
										<button
											onClick={() => {
												setCurrentAffirmationIndex((prev) =>
													prev > 0
														? prev - 1
														: selectedCategory.affirmations.length - 1,
												);
											}}
										...
										<div
											...
											role="tablist"
											aria-label="Affirmation navigation"
										>
										...
										<button
											onClick={() => {
												setCurrentAffirmationIndex((prev) =>
													prev < selectedCategory.affirmations.length - 1
														? prev + 1
														: 0,
												);
											}}
								...
								<div>
									...
									>
										...
										<div
											...
											style={{ borderColor: selectedCategory.borderColor }}
										>
									...
									<div className="flex gap-3">
										<button
											onClick={() => {
												...
												const tips = selectedCategory?.actionableTips || [];
												const availableTips = tips.filter(
													(tip) => tip !== currentTip,
												);
