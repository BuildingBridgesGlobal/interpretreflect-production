import { CheckCircle, ChevronRight, Heart, X } from "lucide-react";
...
export const BodyCheckInAccessible: React.FC<BodyCheckInProps> = ({
	onClose,
	onComplete,
}) => {
				...
				>
					<div className="p-8">
						...
						<div
							...
							role="group"
							aria-label="Body areas progress"
						>
							{areas.map((area, index) => (
								<div
									...
									style={{
										backgroundColor:
											index <= currentIndex ? "#7A9B6E" : "#E2E8F0",
									}}
				...
				<div className="p-8 overflow-y-auto flex-1">
					...
					<fieldset className="mb-6">
						...
						<div className="flex gap-2" role="radiogroup">
							{["Holding a lot", "Some release", "Much lighter"].map(
								(option) => (
									<button
										key={option}
										...
										className={`flex-1 py-2.5 px-3 rounded-xl transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
											tensionLevel === option ? "text-white" : "bg-gray-50"
										}`}
										style={{
											background:
												tensionLevel === option
						...
						<div className="flex gap-2" role="radiogroup">
							{["Depleted", "Okay", "Restored"].map((option) => (
								<button
									...
									className={`flex-1 py-2.5 px-3 rounded-xl transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
										energyLevel === option ? "text-white" : "bg-gray-50"
									}`}
									style={{
										background:
											energyLevel === option
						...
						<div className="flex gap-2" role="radiogroup">
							{["Tight", "Better", "Easeful"].map((option) => (
								<button
									...
									className={`flex-1 py-2.5 px-3 rounded-xl transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
										overallFeeling === option ? "text-white" : "bg-gray-50"
									}`}
									style={{
										background:
							...
							{[
								...
								<button
									...
									className={`p-2.5 rounded-xl text-center transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
										tensionAreas.includes(area) ? "text-white" : "bg-gray-50"
									}`}
									style={{
