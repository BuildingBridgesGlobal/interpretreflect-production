		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to save context reflection",
		};
	}
}
...
function determineContextType(content: Record<string, string>): string {
	...
	if (
		contentStr.includes("medical") ||
		contentStr.includes("patient") ||
		contentStr.includes("clinical")
	) {
		return "medical";
	}
	if (
		contentStr.includes("legal") ||
		contentStr.includes("court") ||
		contentStr.includes("attorney")
	) {
		return "legal";
	}
	if (
		contentStr.includes("education") ||
		contentStr.includes("student") ||
		contentStr.includes("teacher")
	) {
		return "educational";
	}
	if (
		contentStr.includes("therapy") ||
		contentStr.includes("mental health") ||
		contentStr.includes("counseling")
	) {
		return "mental_health";
	}
	if (
		contentStr.includes("community") ||
		contentStr.includes("social") ||
		contentStr.includes("public")
	) {
		return "community";
	}
...
function generateContextInsights(
	...
	insights.recommendations = generateContextRecommendations(
		contextType,
		insights,
	);
	...
	switch (contextType) {
		case "medical":
			...
			if (insights.challenges.some((c) => c.includes("emotional"))) {
				recommendations.push(
					"Practice emotional regulation techniques before medical assignments",
				);
			}
		...
		case "legal":
			...
			if (insights.challenges.some((c) => c.includes("simultaneous"))) {
				recommendations.push(
					"Practice simultaneous interpreting with legal recordings",
				);
			}
		...
		case "educational":
			recommendations.push(
				"Familiarize yourself with grade-level academic vocabulary",
			);
			recommendations.push(
				"Learn about IEP processes and special education terminology",
			);
			if (insights.challenges.some((c) => c.includes("parent"))) {
				recommendations.push(
					"Develop strategies for parent-teacher conference interpreting",
				);
			}
			break;
		...
		case "mental_health":
			...
			recommendations.push(
				"Practice self-care after emotionally intensive sessions",
			);
			if (insights.challenges.some((c) => c.includes("boundaries"))) {
				recommendations.push(
					"Review professional boundary guidelines for mental health settings",
				);
			}
			break;
...
async function updateContextMetrics(
	...
	try {
		...
		await supabase.from("context_metrics").upsert({
			user_id: userId,
			context_type: contextType,
			metrics: scores,
...
function calculateContextScores(
	...
	const overallScore = Math.min(
		100,
		Math.max(0, baseScore + strengthBonus - challengePenalty),
	);

	switch (contextType) {
		case "medical":
			scores.terminologyAccuracy =
				overallScore + (insights.skillsUsed.includes("terminology") ? 10 : 0);
			scores.emotionalManagement =
				overallScore + (insights.skillsUsed.includes("emotional") ? 10 : 0);
		...
		case "legal":
			scores.accuracyScore =
				overallScore + (insights.skillsUsed.includes("accuracy") ? 10 : 0);
			scores.impartialityScore = overallScore;
			scores.technicalProficiency =
				overallScore + (insights.skillsUsed.includes("technical") ? 10 : 0);
		...
		case "educational":
			...
			scores.academicTerminology =
				overallScore + (insights.skillsUsed.includes("terminology") ? 10 : 0);
			break;

		case "mental_health":
			scores.emotionalBoundaries =
				overallScore + (insights.skillsUsed.includes("boundaries") ? 10 : 0);
		...
		case "community":
			scores.culturalCompetence =
				overallScore + (insights.skillsUsed.includes("cultural") ? 10 : 0);
...
export async function prepareContextDataForElya(
	...
	try {
				...
				switch (context) {
					case "medical":
						learningPathSuggestions.push(
							"Advanced Medical Interpreting Certification",
						);
					...
					case "mental_health":
						learningPathSuggestions.push(
							"Mental Health Interpreting Specialization",
						);
						learningPathSuggestions.push(
							"Trauma-Informed Interpreting Training",
						);
						break;
					case "community":
		...
		return {
			success: false,
			error:
				error instanceof Error ? error.message : "Failed to prepare Elya data",
		};
	}
}
