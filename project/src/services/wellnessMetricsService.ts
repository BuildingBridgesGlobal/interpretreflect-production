 ...
 */
function extractMetricsFromReflection(
	entryKind: string,
	data: any,
): {
	...
	if (
		...
		if (data.stressLevel) {
			metrics.stress_level = clampValue(parseFloat(data.stressLevel));
		}
		if (data.energyLevel) {
			metrics.energy_level = clampValue(parseFloat(data.energyLevel));
		}
	}
	...
	if (data.burnoutLevel) {
		metrics.burnout_score = clampValue(parseFloat(data.burnoutLevel));
	...
	if (data.confidence || data.confidenceLevel) {
		metrics.confidence_score = clampValue(
			parseFloat(data.confidence || data.confidenceLevel),
		);
	}
	...
	try {
		...
		if (existing) {
			...
			if (metrics.confidence_score !== undefined) {
				updatedMetrics.confidence_score = clampValue(metrics.confidence_score);
			}
		...
		const reflectionCategory =
			entryKind === "wellness_checkin"
				...
				: entryKind === "post_assignment_debrief"
					? "session_reflection"
					: entryKind === "team_reflection"
						? "team_sync"
						: entryKind === "values_alignment_check"
							? "values_alignment"
							: entryKind === "stress_reduction_technique"
								? "stress_management"
								: "growth_assessment";
