...
/**
 * Create a one-way hash of user ID using Web Crypto API
 ...
 */
async function createSessionHash(sessionId?: string): Promise<string> {
	const encoder = new TextEncoder();
 ...
 */
function anonymizeReflectionData(
	data: Record<string, any>,
): AnonymizedReflection {
	// Map reflection type to category (no free text)
	const getCategory = (
		type: string,
	): AnonymizedReflection["reflection_category"] => {
...
export async function saveReflectionZKWV(
	...
	try {
		...
		const sessionHash = await createSessionHash(data.session_id);

		// Anonymize the data
		const anonymized = anonymizeReflectionData({
			...data,
			reflection_type: reflectionType,
		});

		// Save anonymized reflection
		const { error } = await supabase.from("anonymized_reflections").insert({
			user_hash: userHash,
...
async function updateWellnessMetrics(
	...
	try {
		...
		await supabase.from("wellness_metrics").upsert(wellnessData, {
...
export async function generateWellnessProof(
	...
	try {
		...
		const { data: verified } = await supabase.rpc("verify_wellness_threshold", {
		...
		// Generate proof hash using Web Crypto API
		const encoder = new TextEncoder();
		const proofBuffer = await crypto.subtle.digest("SHA-256", encoder.encode(JSON.stringify(proofData)));
...
export async function getAggregatedInsights(
	...
	try {
		...
		const { data: metrics } = await supabase
			...
			.select(
				"stress_level, energy_level, high_stress_pattern, recovery_needed",
			)
		...
		// Calculate averages
		const avgStress =
			metrics.reduce((sum, m) => sum + (m.stress_level || 0), 0) /
			metrics.length;
		const avgEnergy =
			metrics.reduce((sum, m) => sum + (m.energy_level || 0), 0) /
			metrics.length;
		...
		const trendsDetected = [
			...new Set(patterns?.map((p) => p.pattern_code) || []),
		];
		...
		if (trendsDetected.includes("BURNOUT_RISK"))
			recommendationCodes.push("BURNOUT_PREVENTION_URGENT");
