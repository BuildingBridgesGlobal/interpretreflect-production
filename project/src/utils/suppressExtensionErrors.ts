...
(() => {
	...
	window.onerror = (message, source, lineno, colno, error) => {
		...
		const isExtensionError = EXTENSION_ERROR_PATTERNS.some(
			(pattern) => pattern.test(messageStr) || pattern.test(sourceStr),
		);
	...
	window.onunhandledrejection = (event) => {
		...
		if (isExtensionError) {
			console.debug(
				"Suppressed browser extension promise rejection:",
				reasonStr,
			);
