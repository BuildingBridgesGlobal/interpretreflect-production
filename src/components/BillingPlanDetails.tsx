...
export const BillingPlanDetails: React.FC = () => {
  ...
  const handleUpdatePayment = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('create-update-payment-session', {
        ...
        <div className="bg-white shadow-xl rounded-lg">
          ...
          <div className="px-6 py-6 border-b border-gray-200">
            <div className="flex items-center">
              <FileText className="h-8 w-8 text-indigo-600 mr-3" aria-hidden="true" />
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Billing & Plan Details</h1>
                <p className="text-gray-600 mt-1">
                  Review your payment information and plan history in one place. 
                  We're committed to transparent billing and keeping your information secure at all times.
                </p>
              </div>
            </div>
          </div>
          ...
          {planDetails && (
            <>
              ...
              <section className="px-6 py-6 border-b border-gray-200" aria-labelledby="plan-overview">
                <h2 id="plan-overview" className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <Package className="h-5 w-5 mr-2 text-indigo-600" aria-hidden="true" />
                  Active Plan Overview
                </h2>

                <div className="bg-gray-50 rounded-lg p-6" role="region" aria-label="Current plan details">
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <span className="ml-2 text-lg text-gray-600">
                              {planDetails.price > 0 ? formatPrice(planDetails.price) : ''}
                            </span>
                          </div>
                          <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getStatusColor(planDetails.status)}`}>
                      ...
                      >
                        <div className="flex items-center">
                          <Info className="h-5 w-5 text-indigo-600 mr-2" aria-hidden="true" />
                          <span className="text-sm font-medium text-gray-900">View Plan Features</span>
                        </div>
                        <ChevronRight className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      </button>

                      {/* Trial or Cancellation Notice */}
                      {planDetails.trial_ends_at && planDetails.status === 'trial' && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                          <p className="text-sm text-blue-900">
                            <AlertCircle className="inline h-4 w-4 mr-1" aria-hidden="true" />
                            Trial ends on {formatDate(planDetails.trial_ends_at)}
                          </p>
                        </div>
                      )}
                      ...
                      {planDetails.cancelled_at && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                          <p className="text-sm text-amber-900">
                            <AlertCircle className="inline h-4 w-4 mr-1" aria-hidden="true" />
                            Access continues until {formatDate(planDetails.next_billing_date)}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <button
                        ...
                        disabled={updatingPayment || planDetails.name === 'free'}
              ...
              {/* Invoice History */}
              <section className="px-6 py-6 border-b border-gray-200" aria-labelledby="invoice-history">
                <h2 id="invoice-history" className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <FileText className="h-5 w-5 mr-2 text-indigo-600" aria-hidden="true" />
                  Invoice History
                </h2>

                {invoices.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200" role="table" aria-label="Invoice history table">
              ...
              <section className="px-6 py-6" aria-labelledby="security-notes">
                <h2 id="security-notes" className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <Shield className="h-5 w-5 mr-2 text-indigo-600" aria-hidden="true" />
                  Security & Policy Notes
                </h2>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                  <div className="flex items-start">
                    <Lock className="h-5 w-5 text-blue-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                    <p className="text-sm text-blue-900">
                      All credit card information is securely processed via Stripe and never stored on our servers.
                    </p>
                  </div>
                  
                  <div className="flex items-start">
                    <Shield className="h-5 w-5 text-blue-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                    <p className="text-sm text-blue-900">
                  ...
                  <div className="flex items-start">
                    <Mail className="h-5 w-5 text-blue-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                    <p className="text-sm text-blue-900">
        ...
        >
          <div className="bg-white rounded-lg max-w-md w-full shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 sticky top-0 bg-white">
              <h3 id="features-modal-title" className="text-lg font-semibold text-gray-900">
