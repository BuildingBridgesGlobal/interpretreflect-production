...
import { X, Play, Pause, RotateCcw, Settings, ChevronRight, Check, Circle } from 'lucide-react';
...
type MovementMode = 'top-to-bottom' | 'single-area' | 'jump-around' | 'whole-body';
...
const bodyParts = [
  { id: 'head', name: 'Head and Face', instruction: 'Notice your head and face - relax them if it feels good' },
  { id: 'shoulders', name: 'Shoulders', instruction: 'Check your shoulders - move or adjust them if you want', note: 'Many people hold stress here' },
  { id: 'chest', name: 'Chest area', instruction: 'Feel your chest area - breathe in a comfortable way' },
  { id: 'belly', name: 'Belly', instruction: 'Notice your belly - let it be soft if that\'s okay' },
  { id: 'legs', name: 'Legs and Feet', instruction: 'Feel your legs and feet - relax them or keep them as they are' }
];

export const BodyAwarenessJourneyAccessible: React.FC<BodyAwarenessJourneyAccessibleProps> = ({ 
  onClose, 
  onComplete 
}) => {
  // Core states
  ...
  // Settings
  const [movementMode, setMovementMode] = useState<MovementMode>('top-to-bottom');
      ...
      intervalRef.current = setInterval(() => {
        setTimeElapsed(prev => {
          ...
          if (movementMode === 'top-to-bottom') {
            ...
            if (nextIndex !== currentPartIndex && nextIndex < bodyParts.length) {
  ...
  const handleComplete = () => {
    ...
    if (rememberSettings) {
      localStorage.setItem('bodyAwarenessSettings', JSON.stringify({
        movementMode,
        feelingLevel,
        guidanceLevel,
        highContrast,
    ...
    if (trackProgress) {
      const sessions = JSON.parse(localStorage.getItem('bodyAwarenessSessions') || '[]');
      sessions.push({
  ...
  const getCurrentInstruction = () => {
    ...
    return bodyParts[currentPartIndex]?.instruction || 'Take a moment to get comfortable';
  };

  const getCurrentNote = () => {
    if (movementMode !== 'top-to-bottom' || guidanceLevel === 'myself') return null;
  ...
  if (phase === 'start') {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className={`rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto ${getContainerClass()}`}>
          <div className="p-6 md:p-8">
            {/* Header */}
            <div className="flex justify-between items-start mb-6">
              <div>
                <h1 className={`text-2xl md:text-3xl font-bold mb-2 ${highContrast ? 'text-black' : 'text-gray-900'}`}>
                  Body Awareness Journey
                </h1>
                <p className={`${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-600'}`}>
            ...
            {/* Time selection */}
            <div className="mb-8">
              <label className={`block mb-3 font-medium ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-700'}`}>
                How long:
              </label>
            ...
            {/* Current setting */}
            <div className={`mb-8 p-4 rounded-lg ${highContrast ? 'bg-gray-100 border-2 border-black' : 'bg-blue-50'}`}>
              <p className={`font-medium mb-2 ${getTextSize()}`}>
                Starting with:
              </p>
              <p className={`text-lg font-bold ${highContrast ? 'text-black' : 'text-blue-900'}`}>
                {movementMode === 'top-to-bottom' ? 'Head and Face' :
                 movementMode === 'single-area' ? 'One area you choose' :
                 movementMode === 'jump-around' ? 'Wherever you want' :
                 'Your whole body'}
              </p>
              <p className={`mt-2 ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-700'}`}>
                Begin when ready
              </p>
            </div>
            ...
            {!simpleMode && (
              <div className="mb-8">
                ...
                <ul className={`space-y-2 ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-700'}`}>
                  {bodyParts.map(part => (
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      ...
                      <span className={`ml-2 ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-600'}`}>
                        - {way.desc}
                      </span>
                    </div>
                  ))}
            ...
            <div className={`mb-8 p-4 rounded-lg ${highContrast ? 'bg-yellow-100 border-2 border-black' : 'bg-[rgba(107,130,104,0.05)]'}`}>
              ...
              <p className={`${getTextSize()} ${highContrast ? 'text-black' : 'text-green-900'}`}>
                Paying attention to your body helps your brain get better at noticing feelings and managing stress.
              </p>
            </div>

            {/* If you need to change something */}
            <div className={`mb-8 p-4 rounded-lg ${highContrast ? 'bg-gray-100 border-2 border-black' : 'bg-amber-50'}`}>
              <h2 className={`font-medium mb-2 ${getTextSize()}`}>
                If you need to change something:
              </h2>
              <ul className={`space-y-1 ${getTextSize()} ${highContrast ? 'text-black' : 'text-amber-900'}`}>
            ...
            {!simpleMode && (
              <div className="mb-8">
                ...
                <ul className={`space-y-1 ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-700'}`}>
            ...
            {/* Settings panel */}
            {showSettings && (
              <div className={`mt-6 p-6 rounded-lg ${highContrast ? 'bg-gray-100 border-2 border-black' : 'bg-gray-50'}`}>
                <h2 className={`text-xl font-bold mb-4 ${highContrast ? 'text-black' : 'text-gray-900'}`}>
                  Settings Menu
                </h2>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {[
                      { id: 'top-to-bottom' as MovementMode, label: 'Top to bottom (head to feet)' },
                      { id: 'single-area' as MovementMode, label: 'Stay with one area' },
                      { id: 'jump-around' as MovementMode, label: 'Jump around' },
                      { id: 'whole-body' as MovementMode, label: 'Whole body at once' }
                    ].map(mode => (
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {[
                      { id: 'strong' as FeelingLevel, label: 'Strong feelings (more pressure/movement)' },
                      { id: 'light' as FeelingLevel, label: 'Light feelings (gentle/calm)' },
                      { id: 'figure-out' as FeelingLevel, label: "I'll figure it out as I go" }
                    ].map(level => (
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {[
                      { id: 'full' as GuidanceLevel, label: 'Tell me what to do' },
                      { id: 'little' as GuidanceLevel, label: 'Just a little guidance' },
                      { id: 'myself' as GuidanceLevel, label: 'Let me do it myself' }
                    ].map(level => (
                      <label key={level.id} className="flex items-center">
                        <input
                          type="radio"
                  ...
                  <div className="space-y-2">
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-3"
                      />
                      <span className={getTextSize()}>Simple mode (less information)</span>
                    </label>
                  </div>
                </div>
        ...
        <div className={`rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto ${getContainerClass()}`}>
          <div className="p-6 md:p-8">
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl md:text-2xl font-bold ${highContrast ? 'text-black' : 'text-gray-900'}`}>
                Now: {getCurrentPart()}
              </h2>
              <button 
                onClick={onClose}
            ...
            {/* Note if available */}
            {getCurrentNote() && (
              <p className={`mb-4 ${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-600'}`}>
                {getCurrentNote()}
              </p>
            )}
            ...
            <div className={`mb-8 p-8 rounded-xl text-center ${
              highContrast ? 'bg-gray-100 border-2 border-black' : 'bg-blue-50'
            }`}>
              <p className={`text-lg md:text-xl ${highContrast ? 'text-black font-bold' : 'text-gray-800'}`}>
            ...
            <div className="mb-8 flex justify-center" aria-hidden="true">
              <svg width="150" height="250" viewBox="0 0 150 250" className="max-w-full">
                {/* Head */}
                <circle 
                  cx="75" 
                  cy="35" 
                  r="25" 
                  fill={currentPartIndex === 0 && movementMode === 'top-to-bottom' ? 
                    (highContrast ? '#000000' : '#3B82F6') : 
                ...
                <rect 
                  ...
                  width="50" 
                  height="80" 
                  fill={(currentPartIndex === 2 || currentPartIndex === 3) && movementMode === 'top-to-bottom' ? 
                    (highContrast ? '#000000' : '#3B82F6') : 
                    (highContrast ? '#FFFFFF' : '#E5E7EB')}
                  stroke={highContrast ? '#000000' : '#9CA3AF'}
                  strokeWidth="2"
                ...
                <line 
                  ...
                  x2="20" 
                  y2="120" 
                  stroke={currentPartIndex === 1 && movementMode === 'top-to-bottom' ? 
                    (highContrast ? '#000000' : '#3B82F6') : 
                    (highContrast ? '#000000' : '#9CA3AF')}
                  strokeWidth="3"
                />
                <line 
                  x1="100" 
                ...
                <line 
                  ...
                  x2="55" 
                  y2="220" 
                  stroke={currentPartIndex === 4 && movementMode === 'top-to-bottom' ? 
                    (highContrast ? '#000000' : '#3B82F6') : 
                    (highContrast ? '#000000' : '#9CA3AF')}
                  strokeWidth="3"
                />
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <span className={`${getTextSize()} font-medium`}>Progress:</span>
                <span className={`${getTextSize()}`}>
                  {Math.floor(timeElapsed / 60)}:{(timeElapsed % 60).toString().padStart(2, '0')}
                </span>
              </div>
              <div className="flex gap-1" role="progressbar" aria-valuenow={getProgress()} aria-valuemin={0} aria-valuemax={100}>
                {[...Array(10)].map((_, i) => (
                  <div
                    key={i}
                    className={`h-4 flex-1 rounded ${
                      i < Math.floor(getProgress() / 10) 
                        ? (highContrast ? 'bg-black' : 'bg-blue-600')
                        : (highContrast ? 'bg-gray-300 border border-black' : 'bg-gray-200')
                    }`}
                  />
                ))}
              </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto ${getContainerClass()}`}>
        <div className="p-6 md:p-8">
          <h2 className={`text-2xl md:text-3xl font-bold mb-6 ${highContrast ? 'text-black' : 'text-gray-900'}`}>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className={`font-medium ${getTextSize()}`}>
                    {energyLevel === 1 ? 'Low' : energyLevel === 10 ? 'High' : energyLevel}
                  </span>
                </div>
                <input
                  type="range"
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className={`font-medium ${getTextSize()}`}>
                    {calmLevel === 1 ? 'Low' : calmLevel === 10 ? 'High' : calmLevel}
                  </span>
                </div>
                <input
                  type="range"
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className={`font-medium ${getTextSize()}`}>
                    {focusLevel === 1 ? 'Low' : focusLevel === 10 ? 'High' : focusLevel}
                  </span>
                </div>
                <input
                  type="range"
          ...
          {timeElapsed > 30 && (
            <div className={`mb-6 p-4 rounded-lg ${
              ...
              <p className={`${getTextSize()} ${highContrast ? 'text-black' : 'text-gray-700'}`}>
          ...
          <div className="mb-6 space-y-2">
            ...
            <label className="flex items-center">
              <input
                ...
                className="mr-3"
              />
              <span className={getTextSize()}>Track how my body feels over time</span>
            </label>
