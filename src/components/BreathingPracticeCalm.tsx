...
export const BreathingPractice: React.FC<BreathingPracticeProps> = ({ onClose, onComplete }) => {
  ...
  const [selectedRhythm, setSelectedRhythm] = useState<BreathingRhythm>('steady');
  ...
  const getRhythmTiming = () => {
    switch (selectedRhythm) {
      ...
      case 'natural': return { in: 0, out: 0 }; // No guided timing
        ...
        <div className="rounded-3xl max-w-lg w-full bg-white shadow-sm">
          <div className="p-8">
            ...
            <div className="mb-8">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { value: 'calming', label: 'Calming', desc: 'Longer exhales' },
                  { value: 'natural', label: 'Natural', desc: 'Your own pace' },
                  { value: 'gentle', label: 'Gentle', desc: 'Soft and easy' }
                ].map(rhythm => (
                  <button
                    key={rhythm.value}
                    onClick={() => setSelectedRhythm(rhythm.value as BreathingRhythm)}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm text-gray-400">{rhythm.desc}</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <p className="text-sm text-gray-400 mb-8 text-center">
              Simply breathing steadily for 30 seconds begins to calm your nervous system.
            </p>
  ...
  if (phase === 'practice') {
    const timing = getRhythmTiming();
    const breathingScale = selectedRhythm === 'natural' ? 1 : (breathPhase === 'in' ? 1.15 : 0.85);
    ...
    const getDurationText = () => {
      switch (selectedDuration) {
        case '30s': return '';
        case '1m': return '';
        case '2m': return '2 minutes of calm';
        case '4m': return '';
        default: return '';
      }
    };
