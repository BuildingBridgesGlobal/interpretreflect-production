...
const questions = [
  {
    ...
    options: [
      { value: 5, label: 'Full tank - energized and ready', description: 'Feeling recharged' },
      { value: 4, label: 'Three quarters - good to go', description: 'Solid energy' },
      { value: 3, label: 'Half tank - managing okay', description: 'Moderate energy' },
      { value: 2, label: 'Running low - pushing through', description: 'Energy depleting' },
  ...
  },
  {
    ...
    options: [
      { value: 5, label: 'Quickly - refreshed after short breaks', description: 'Fast recovery' },
      { value: 4, label: 'Well - decent recharge from rest', description: 'Good recovery' },
      { value: 3, label: 'Slowly - need longer to feel better', description: 'Slow recovery' },
      { value: 2, label: 'Barely - breaks don\'t help much', description: 'Poor recovery' },
      { value: 1, label: 'Not at all - still tired after rest', description: 'No recovery' },
    ],
  },
  {
    id: 'emotionalLeakage',
    ...
    options: [
      { value: 5, label: 'Leave them at work - clear separation', description: 'Strong boundaries' },
      { value: 4, label: 'Mostly contained - rare intrusions', description: 'Good boundaries' },
      { value: 3, label: 'Some spillover - occasional rumination', description: 'Some spillover' },
      { value: 2, label: 'Often intrude - hard to disconnect', description: 'Frequent spillover' },
      { value: 1, label: 'Constant - can\'t stop thinking about work', description: 'No boundaries' },
    ],
  },
  {
    ...
    options: [
      { value: 5, label: 'Sharp - at my best', description: 'Peak performance' },
      { value: 4, label: 'Strong - doing well', description: 'Good performance' },
      { value: 3, label: 'Adequate - getting by', description: 'Acceptable level' },
      { value: 2, label: 'Slipping - making mistakes', description: 'Declining quality' },
      { value: 1, label: 'Struggling - can\'t focus', description: 'Poor performance' },
    ],
  },
  {
    ...
    options: [
      { value: 5, label: 'Excited - looking forward to it', description: 'Positive anticipation' },
      { value: 4, label: 'Ready - prepared to tackle it', description: 'Feeling prepared' },
      { value: 3, label: 'Neutral - just another day', description: 'Neither positive nor negative' },
      { value: 2, label: 'Reluctant - not looking forward', description: 'Some dread' },
      { value: 1, label: 'Dreading - wish I could skip it', description: 'Strong avoidance' },
    ],
  },
];

const getRiskLevel = (score: number): 'low' | 'moderate' | 'high' | 'severe' => {
  ...
  switch (level) {
    ...
    case 'moderate':
      return {
        ...
        description: 'Some burnout indicators are elevated. Consider increasing recovery activities',
        icon: Activity,
        color: '#8B7355',
        bgColor: '#FFF9F0',
        borderColor: '#C4A57B'
    ...
    case 'high':
      return {
        title: 'Action Needed',
        description: 'Multiple burnout indicators are concerning. Priority recovery required',
        icon: AlertCircle,
        color: '#C87137',
        bgColor: '#FFF5EE',
        borderColor: '#E09456'
    ...
    case 'severe':
      return {
        ...
        description: 'Burnout risk is severe. Immediate intervention recommended',
        icon: AlertCircle,
        color: '#B85450',
        bgColor: '#FFF0F0',
        borderColor: '#D67572'
...
export const DailyBurnoutGaugeAccessible: React.FC<DailyBurnoutGaugeProps> = ({ onComplete, onClose }) => {
  const { user } = useAuth();
  ...
  const getRecommendations = (answersObj: typeof answers, level: string): string[] => {
    const recs = [];
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          total_score: results.totalScore.toString(),
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-start mb-6">
              <h2 id="results-heading" className="text-2xl font-bold" style={{ color: '#2D3748' }}>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                >
                  <LevelIcon className="h-8 w-8" style={{ color: levelDetails.color }} />
                </div>
                <div className="flex-1">
                  <h3 id="risk-level-heading" className="text-xl font-bold mb-2" style={{ color: levelDetails.color }}>
                    {levelDetails.title}
                  </h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-bold" style={{ color: value >= 3 ? '#5C7F4F' : '#C87137' }}>
                          {value}/5
                        </span>
                        <span className="text-sm" style={{ color: '#718096' }}>
                          ({value >= 4 ? 'Good' : value >= 3 ? 'Fair' : value >= 2 ? 'Low' : 'Critical'})
                        </span>
                      </div>
                    </li>
                  );
              ...
              <ul className="space-y-2" role="list">
                {getRecommendations(answers, level).map((rec, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <CheckCircle className="h-5 w-5 mt-0.5 flex-shrink-0" style={{ color: '#5C7F4F' }} />
                    <span className="text-sm" style={{ color: '#4A5568' }}>{rec}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      >
        <form ref={formRef} className="p-6" onSubmit={(e) => e.preventDefault()}>
          <div className="flex justify-between items-center mb-6">
            <h2 id="burnout-gauge-heading" className="text-2xl font-bold" style={{ color: '#2D3748' }}>
              Daily Burnout Gauge
            </h2>
            <button
              type="button"
          ...
          <p id="burnout-description" className="text-sm mb-6" style={{ color: '#4A5568' }}>
          ...
          <fieldset className="mb-6">
            <legend className="flex items-center gap-3 text-lg font-semibold mb-3" style={{ color: '#2D3748' }}>
              ...
              {currentQ.options.map((option) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      borderColor: answers[currentQ.id as keyof typeof answers] === option.value ? '#5C7F4F' : '#CBD5E0'
                    }}
                  >
                    {answers[currentQ.id as keyof typeof answers] === option.value && (
                      <div
                  ...
                  <div className="flex-1">
                    ...
                    <div id={`${currentQ.id}-${option.value}-desc`} className="text-sm mt-1" style={{ color: '#718096' }}>
                      {option.description}
                    </div>
                  </div>
                  <span
                    ...
                    style={{
                      backgroundColor: option.value >= 4 ? '#F0F5ED' : option.value >= 3 ? '#FFF9F0' : '#FFF5EE',
                      color: option.value >= 4 ? '#5C7F4F' : option.value >= 3 ? '#8B7355' : '#C87137'
                    }}
                  >
                    {option.value}/5
                  </span>
