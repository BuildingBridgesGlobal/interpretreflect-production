...
${elyaData.skillProfile.slice(0, 3)
  .map(skill => `- ${skill.skill} (used ${skill.frequency} times)`)
  .join('\n')}

Top Challenges:
${elyaData.topChallenges.slice(0, 3)
  .map(challenge => `- ${challenge.challenge}`)
  .join('\n')}

Recommended Learning Paths:
${elyaData.learningPathSuggestions.slice(0, 3)
  .map(path => `- ${path}`)
  .join('\n')}
  ...
  if (!statistics) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500 mt-2">Complete context-specific reflections to see insights here</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Context-Specific Insights</h2>
      ...
      <div className="flex gap-2 p-1 bg-gray-100 rounded-lg w-fit">
        {(['week', 'month', '90days'] as const).map((period) => (
          ...
          >
            {period === 'week' ? 'Week' : period === 'month' ? 'Month' : '90 Days'}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {Object.entries(statistics.byContext).map(([context, count]) => (
            <div
              ...
              style={{
                borderColor: selectedContext === context ? contextColors[context] : '#E5E7EB',
                backgroundColor: selectedContext === context ? `${contextColors[context]}10` : 'white'
              }}
              onClick={() => setSelectedContext(context === selectedContext ? 'all' : context)}
            >
              <div className="flex justify-center mb-2" style={{ color: contextColors[context] }}>
                {contextIcons[context] || <Target className="w-5 h-5" />}
              </div>
              <div className="text-2xl font-bold text-gray-900">{count}</div>
              <div className="text-sm text-gray-600 capitalize">{context.replace('_', ' ')}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {selectedContext !== 'all' && statistics.contextSpecificMetrics[selectedContext] && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <BarChart3 className="w-5 h-5" style={{ color: contextColors[selectedContext] }} />
            {selectedContext.replace('_', ' ')} Performance Metrics
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(statistics.contextSpecificMetrics[selectedContext]).map(([metric, score]) => (
              <MetricCard
                key={metric}
              ...
              {statistics.topChallenges.map((challenge, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-gray-700 text-sm">{challenge.challenge}</p>
              ...
              {statistics.topSkills.map((skill, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-700 capitalize">{skill.skill}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500">{skill.frequency}x</span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500">Skills will appear as you complete more reflections</p>
          )}
        </div>
      </div>
        ...
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 rounded-xl p-6 border border-purple-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {elyaData.learningPathSuggestions.slice(0, 4).map((path, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <span className="text-purple-600 mt-0.5">→</span>
          ...
          <div className="mt-4 pt-4 border-t border-purple-200">
            <p className="text-sm text-gray-600">
              Based on analysis of {statistics.totalReflections} context-specific reflections
            </p>
          </div>
        </div>
      )}
      ...
      {statistics.growthTrajectory && statistics.growthTrajectory.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
