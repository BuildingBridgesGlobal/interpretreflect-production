...
// ========== MAIN COMPONENT ==========
export const GrowthInsightsDashboard: React.FC = () => {
  const { user } = useAuth();
  ...
  const [communityAverage, setCommunityAverage] = useState<Record<string, number>>({});
  ...
  const fetchEmotions = async () => {
    ...
    const todayEntry = emotions.find(e => 
  ...
  const saveEmotion = async (value: 'red' | 'amber' | 'green', emoji?: string) => {
    try {
      ...
      // Show success toast
      showSuccessToast(`Mood logged: ${EMOTION_CONFIG[value].label} ${emoji || ''}`);
      
      // Announce to screen readers
      announceToScreenReader(`Your mood has been logged as ${EMOTION_CONFIG[value].label}`);
  ...
  const processMetrics = (data: any) => {
    ...
    const weekReflections = reflections.filter((r: any) => 
      new Date(r.created_at) >= oneWeekAgo
    ).length;
    const monthReflections = reflections.filter((r: any) => 
      new Date(r.created_at) >= oneMonthAgo
    ).length;

    // Calculate wellness average
    const wellnessScores = wellness.map((w: any) => (w.data as any)?.overall_wellbeing || 5);
    const avgWellness = wellnessScores.length > 0 
      ? wellnessScores.reduce((a: number, b: number) => a + b, 0) / wellnessScores.length
      : 5;

    // Last reflection check
    const lastReflection = reflections[0];
    const daysSinceLastReflection = lastReflection 
      ? Math.floor((now.getTime() - new Date(lastReflection.created_at).getTime()) / 86400000)
      : null;
    ...
    const metricsData: Metric[] = [
      {
        ...
        cta: daysSinceLastReflection && daysSinceLastReflection > 2 ? {
      ...
      {
        label: 'Team Sessions',
        value: teaming.filter((t: any) => t.entry_kind.includes('teaming')).length,
        icon: <Users className="w-6 h-6" />,
        color: '#8B5CF6',
        bgColor: '#EDE9FE',
      ...
      {
        label: 'Values Checks',
        value: teaming.filter((t: any) => t.entry_kind === 'compass_check').length,
        icon: <Shield className="w-6 h-6" />,
        color: '#14B8A6',
        bgColor: '#CCFBF1',
  ...
  // ========== GAMIFICATION ==========
  const updateBadges = (reflections: any[], wellness: any[], teaming: any[], streaks: any) => {
    ...
    // Boundary Setter
    const boundaryCount = reflections.filter((r: any) => 
      r.entry_kind === 'compass_check'
    ).length;
    if (boundaryCount >= 5) {
  ...
  const renderEmotionRAG = () => (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 id="emotion-rag-heading" className="text-xl font-semibold text-gray-900">
            Daily Mood Check-in
          </h2>
          <p className="text-sm text-gray-600 mt-1">
            {todayEmotion ? 'Today\'s mood logged' : 'How are you feeling today?'}
          </p>
        </div>
        <div className="relative group">
          ...
          <div className="absolute right-0 top-full mt-2 w-64 p-3 bg-gray-900 text-white text-sm rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
            Regular mood tracking helps identify patterns and triggers in your wellness journey
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {Object.entries(EMOTION_CONFIG).map(([key, config]) => (
          <button
            key={key}
            onClick={() => saveEmotion(key as 'red' | 'amber' | 'green', config.emoji)}
          ...
          >
            <div className="flex flex-col items-center">
              <span className="text-2xl mb-2" role="img" aria-label={config.label}>
                {config.emoji}
              </span>
              {React.cloneElement(config.icon as React.ReactElement, {
                style: { color: config.color }
              })}
              <span className="text-sm font-medium mt-2" style={{ color: config.color }}>
                {config.label}
              </span>
            </div>
          </button>
      ...
      <div>
        ...
        <div className="flex gap-1" role="list" aria-label="Weekly mood history">
          {[...Array(7)].map((_, index) => {
            const date = new Date();
            date.setDate(date.getDate() - (6 - index));
            const dateStr = date.toDateString();
            const emotion = weekEmotions.find(e => 
    ...
    >
      {metrics.map((metric, index) => (
        ...
        >
          ...
          <h3 id={`metric-${index}-heading`} className="text-2xl font-bold text-gray-900">
            {metric.value}
          </h3>
  ...
  const renderBadges = () => (
    ...
    >
      <h2 id="badges-heading" className="text-xl font-semibold text-gray-900 mb-4">
  ...
  const renderActivityFeed = () => (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h2 id="activity-heading" className="text-xl font-semibold text-gray-900">
          ...
          {activities.map(activity => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <time className="text-xs text-gray-500 mt-1" dateTime={activity.timestamp}>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center p-6 bg-red-50 rounded-lg max-w-md" role="alert">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6 text-center">
          <Sparkles className="w-8 h-8 text-purple-600 mx-auto mb-3" aria-hidden="true" />
          <h2 className="text-lg font-semibold text-gray-900 mb-2">
            Today's Micro-habit
          </h2>
          <p className="text-gray-700 mb-4">
            Take 5 minutes for a quick reflection. Small steps lead to big changes.
          </p>
          <button
            onClick={handleStartReflection}
