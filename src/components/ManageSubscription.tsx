...
const PLAN_DETAILS = {
  ...
  professional: {
    ...
    price: 24,
    features: ['Everything in Essential', 'Team features', 'Priority support', 'Advanced analytics']
  },
  organization: {
    display: 'Organization',
    price: null,
    features: ['Custom pricing', 'Enterprise features', 'Dedicated support', 'Custom integrations']
  }
};
...
export const ManageSubscription: React.FC = () => {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      announcement.textContent = 'Your subscription has been cancelled. You will retain access until the end of your billing period.';
  ...
  const StatusIcon = subscription ? getStatusIcon(subscription.status) : AlertCircle;
        ...
        <div className="bg-white shadow-xl rounded-lg">
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Easily control your plan and billing options. Change, cancel, or review your current plan anytime, hassle-free.
                  </p>
                </div>
              </div>
            </div>
          ...
          {subscription && (
            <>
              ...
              <section className="px-6 py-6 border-b border-gray-200" aria-labelledby="current-plan">
                <h2 id="current-plan" className="text-lg font-semibold text-gray-900 mb-4">Current Plan Summary</h2>
                
                <div className="bg-gray-50 rounded-lg p-6" role="region" aria-label="Subscription details">
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="mt-1 flex items-center">
                          ...
                          />
                          <span className={`ml-2 text-sm ${getStatusColor(subscription.status)}`}>
                    ...
                    <div className="space-y-4">
                      {subscription.payment_method ? (
                        <div>
                          <label className="block text-sm font-medium text-gray-700">
                            <CreditCard className="inline h-4 w-4 mr-1" aria-hidden="true" />
                            Payment Method
                          </label>
                          <div className="mt-1 flex items-center">
                            ...
                            <span className="sr-only">
                              {subscription.payment_method.brand} card ending in {subscription.payment_method.last4}
                            </span>
                          </div>
                        </div>
                      ) : (
                        <div>
                          ...
                          <p className="mt-1 text-gray-500">No payment method on file</p>
                        </div>
                      )}
                      ...
                      {subscription.cancelled_at && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                          <p className="text-sm text-amber-900">
                            <AlertCircle className="inline h-4 w-4 mr-1" aria-hidden="true" />
                            Access continues until {formatDate(subscription.next_billing_date)}
                          </p>
                        </div>
                      )}
                    </div>
              ...
              {/* Actions */}
              <section className="px-6 py-6 border-b border-gray-200" aria-labelledby="subscription-actions">
                <h2 id="subscription-actions" className="text-lg font-semibold text-gray-900 mb-4">Actions</h2>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  {subscription.status === 'active' && subscription.plan_name !== 'free' && (
                    ...
                    >
                      <div className="flex items-center">
                        <XCircle className="h-5 w-5 text-red-600 mr-3" aria-hidden="true" />
                        <div className="text-left">
          ...
          <section id="invoices-section" className="px-6 py-6 border-b border-gray-200" aria-labelledby="invoices">
            ...
            {invoices.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200" role="table" aria-label="Invoice history">
              ...
              <p className="text-gray-500 text-center py-8">No invoices available yet.</p>
            )}
          </section>
          ...
          <section className="px-6 py-6 border-b border-gray-200" aria-labelledby="policies">
            ...
            <ul className="space-y-3" role="list">
              <li className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                <span className="text-gray-700">No long-term contracts. Cancel anytime.</span>
              </li>
              <li className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                <span className="text-gray-700">7-day money-back guarantee for first-time subscribers.</span>
              </li>
              <li className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-600 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
          ...
          <section className="px-6 py-6" aria-labelledby="security">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-900 mb-2">
                All billing is securely processed through Stripe. PCI-compliant, never stored on InterpretReflect servers.
              </p>
              <p className="text-sm text-blue-900">
                All actions and information are screen reader-friendly, fully keyboard navigable, with visible focus indicators.
                High-contrast tables/cards, clear headings, and large, labeled action buttons ensure accessibility for all users.
              </p>
            </div>
          </section>
        </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full mx-4 shadow-xl">
            ...
            <div className="px-6 py-4">
              <div className="flex items-start mb-4">
                <AlertCircle className="h-6 w-6 text-amber-500 mt-0.5 mr-3 flex-shrink-0" aria-hidden="true" />
                <div>
                  <p className="text-gray-700">
                    Are you sure you want to cancel? You will retain access until{' '}
                    <strong>{subscription?.next_billing_date ? formatDate(subscription.next_billing_date) : 'the end of your billing period'}</strong>.
                  </p>
                  <p className="text-sm text-gray-600 mt-2">
                    Your reflections remain secure and exportable even after canceling.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
              ...
              <button
                ...
                aria-label={cancelling ? 'Cancelling subscription...' : 'Confirm cancellation'}
              >
