...
export function OnboardingFlow({ onComplete, onClose }: OnboardingFlowProps) {
	...
	const interpreterTypes = [
		...
		{
			id: "legal",
			label: "Legal/Court Interpreter",
			icon: <Shield className="w-5 h-5" />,
		},
		{
			id: "educational",
			label: "Educational Interpreter",
			icon: <Brain className="w-5 h-5" />,
		},
		{
			id: "conference",
			label: "Conference Interpreter",
			icon: <CommunityIcon size={20} />,
		},
		{
			id: "community",
			label: "Community Interpreter",
			icon: <CommunityIcon size={20} />,
		},
	...
	const goals = [
		...
		{
			id: "selfcare",
			label: "Develop consistent self-care habits",
			icon: <GrowthIcon size={16} />,
		},
		{
			id: "processing",
			label: "Better process difficult assignments",
			icon: <NotepadIcon size={16} />,
		},
		{
			id: "community",
			label: "Connect with other interpreters",
		...
		{
			id: "balance",
			label: "Achieve better work-life balance",
			icon: <HourglassPersonIcon size={16} />,
		},
		{
			id: "wellness",
			...
			<div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
				...
				<div className="p-6">
					{currentStep === 0 && (
						<div className="text-center">
							...
							<p className="text-gray-600 mb-6">
								We'll ask a few quick questions to customize InterpretReflect
								for your unique needs as an interpreter. This will help us
								provide the most relevant tools and insights for your wellness
								journey.
							</p>
					...
					{currentStep === 1 && (
						<div>
							<h3 className="font-semibold mb-4">
								What type of interpreting do you primarily do?
							</h3>
							<div className="space-y-3 mb-6">
								{interpreterTypes.map((type) => (
									...
									>
										<div
											className={`${profile.interpreter_type === type.id ? "text-green-600" : "text-gray-400"}`}
										>
											{type.icon}
										</div>
					...
					{currentStep === 2 && (
						<div>
							<h3 className="font-semibold mb-4">
								What challenges would you like support with? (Select all that
								apply)
							</h3>
					...
					{currentStep === 4 && (
						<div className="space-y-6">
							<div>
								<h3 className="font-semibold mb-4">
									How long do you prefer your wellness sessions?
								</h3>
								<div className="space-y-3">
									{sessionLengths.map((length) => (
										<button
											...
											onClick={() =>
												setProfile((prev) => ({
													...prev,
													preferred_session_length: length.id,
												}))
											}
								...
								<div className="space-y-3">
									...
									<label className="flex items-center space-x-3">
										<input
											type="checkbox"
											checked={
												profile.notification_preferences.emergency_support
											}
											onChange={(e) =>
												setProfile((prev) => ({
													...prev,
					...
					{currentStep === 5 && (
						<div className="text-center">
							...
							<p className="text-gray-600 mb-6">
								Based on your responses, we've customized InterpretReflect to
								support your unique needs. You can always update these
							...
							<div className="bg-gray-50 rounded-lg p-4 mb-6">
								...
								<ul className="text-sm text-gray-600 space-y-1">
									<li>
										• {profile.preferred_session_length}-minute wellness
										sessions
									</li>
									<li>
										• Tools focused on {profile.primary_challenges.length} key
										challenge areas
									</li>
									<li>
										• Resources for {profile.interpreter_type.replace("_", " ")}{" "}
										interpreters
									</li>
									<li>
										• Progress tracking toward {profile.wellness_goals.length}{" "}
										wellness goals
									</li>
								</ul>
							</div>
				...
				<div className="p-6 border-t bg-gray-50 flex justify-between">
					...
					{currentStep === steps.length - 1 ? (
						...
						<button
							...
							disabled={
								(currentStep === 1 &&
									(!profile.interpreter_type || !profile.experience_level)) ||
								(currentStep === 2 &&
									profile.primary_challenges.length === 0) ||
