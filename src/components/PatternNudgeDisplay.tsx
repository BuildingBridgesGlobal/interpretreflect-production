...
import { X, ChevronRight, TrendingUp, AlertCircle, Sparkles, Heart, Target, Clock } from 'lucide-react';
import { 
...
export const PatternNudgeDisplay: React.FC<PatternNudgeDisplayProps> = ({
  ...
  const getSampleUserData = (): UserData => {
    ...
    return {
      emotions: [
        { emotion: 'stressed', intensity: 4, timestamp: now, context: { assignmentType: 'medical', timeOfDay: 'afternoon' } },
        { emotion: 'exhausted', intensity: 4, timestamp: dayAgo, context: { assignmentType: 'medical', timeOfDay: 'evening' } },
        { emotion: 'anxious', intensity: 3, timestamp: weekAgo, context: { dayOfWeek: 'Monday', timeOfDay: 'morning' } }
      ],
      assignments: [
        { type: 'medical', duration: 180, difficulty: 'challenging', timestamp: dayAgo, completed: true },
        { type: 'legal', duration: 120, difficulty: 'moderate', timestamp: weekAgo, completed: true }
      ],
      resets: [
        { type: 'breathwork', timestamp: now, effectiveness: 4, skipped: false },
        { type: 'boundaries', timestamp: dayAgo, skipped: true, reason: 'no time' }
      ],
      wellnessActions: [
        { action: 'meditation', category: 'mindfulness', timestamp: now, duration: 10, effectiveness: 4 }
      ],
      currentStreak: 7,
      lastResetTime: now
    };
            ...
            >
              ...
              <div className="pr-8">
                ...
                <div className="flex items-start gap-3 mb-2">
                  <div className={`
                    p-2 rounded-full
                    ${nudge.priority === 'high' ? 'bg-red-100 text-red-600' :
                      nudge.priority === 'medium' ? 'bg-yellow-100 text-yellow-600' :
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-900 flex items-center gap-2">
                      ...
                      {nudge.icon && <span className="text-lg">{nudge.icon}</span>}
                    </h4>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <div className="mt-3">
                      ...
                      <ul className="text-xs text-gray-600 space-y-1">
                        {engine.getPersonalizedRecommendations().slice(0, 2).map((rec, idx) => (
 ...
 */
export const PatternNudgeWidget: React.FC<{ userName?: string }> = ({ userName }) => {
  return (
