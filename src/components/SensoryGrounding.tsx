...
import { X, Play, Pause, RotateCcw, Heart, Eye, Ear, Hand, Wind, Brain, Sparkles, Cloud, Mountain } from 'lucide-react';
...
export const SensoryGrounding: React.FC<SensoryGroundingProps> = ({ onClose, onComplete }) => {
  ...
  const currentStepData = currentApproach.steps[Math.min(currentStep, currentApproach.steps.length - 1)];
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div className="p-8">
            ...
            <p className="text-gray-600 mb-6">Your feedback helps us personalize your experience</p>

            {/* Grounding Effect */}
            <div className="mb-6">
              <p className="font-semibold mb-3">How grounded do you feel now?</p>
              <div className="space-y-2">
                {[
                  ...
                  <label key={option} className="flex items-center p-3 rounded-lg border cursor-pointer hover:bg-gray-50">
                    <input
                      type="radio"
            ...
            <div className="mb-6">
              ...
              >
                ...
                <option value="having-choices">Having multiple options to choose from</option>
                <option value="no-pressure">No pressure to use specific senses</option>
                <option value="flexible-timing">Being able to go at my own pace</option>
                <option value="mental-options">Mental/imagination options</option>
            ...
            {/* Adjustments */}
            <div className="mb-6">
              <p className="font-semibold mb-3">Any adjustments for next time?</p>
              <textarea
        ...
        <div className="p-8">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-gray-600">Ground yourself in ways that work for you</p>
            </div>
          ...
          {/* Approach Selector */}
          <div className="mb-6">
            <p className="text-sm font-semibold mb-3">Choose your grounding approach:</p>
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-lg border text-center ${
                      selectedApproach === approach.id ? 'bg-green-50 border-green-500' : 'border-gray-300'
                    }`}
                  >
                    <Icon className="w-6 h-6 mx-auto mb-2" style={{ 
                      color: selectedApproach === approach.id ? '#5C7F4F' : '#9CA3AF' 
                    }} />
                    <p className="font-medium text-sm">{approach.title}</p>
                    <p className="text-xs text-gray-500 mt-1">{approach.description}</p>
                  </button>
                );
              })}
            </div>
          ...
          {currentStepData && (
            <div className="mb-6">
              <div className="mb-4 p-6 rounded-xl" style={{ backgroundColor: '#F8FBF6' }}>
                <p className="text-lg font-semibold mb-4" style={{ color: '#2D5F3F' }}>
                  Step {currentStep + 1}: {currentStepData.title}
                </p>
                <div className="space-y-2">
                  <p className="text-sm text-gray-600 mb-3">Choose any that work for you (or create your own):</p>
                  {currentStepData.options.map((option, idx) => (
                    <label key={idx} className="flex items-start p-3 rounded-lg bg-white border border-gray-200 cursor-pointer hover:bg-gray-50">
                      <input
                        type="checkbox"
              ...
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">Or do something else that helps you:</p>
                <input
                  type="text"
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && e.currentTarget.value) {
                      setCompletedSteps([...completedSteps, e.currentTarget.value]);
          ...
          <div className="mb-6 p-4 rounded-lg bg-blue-50 border border-blue-200">
            <p className="text-sm font-semibold text-blue-900 mb-2">Remember:</p>
          ...
          <div className="flex justify-center gap-4">
            ...
            >
              {currentStep < currentApproach.steps.length - 1 ? 'Next Step' : 'Finish'}
            </button>
          </div>
