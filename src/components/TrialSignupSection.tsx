...
export function TrialSignupSection({ onTrialStarted }: TrialSignupSectionProps) {
  const { user } = useAuth();
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      // Start the trial
      const { data, error: trialError } = await supabase.rpc('start_user_trial', {
        user_id: user.id
      });
  ...
  return (
    <>
      <div className="rounded-3xl p-8 md:p-12 shadow-xl" style={{ background: 'linear-gradient(to bottom right, #F5F9F5, #E8F5E8)' }}>
        {/* Header with urgency */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold mb-4" style={{ backgroundColor: '#E8F5E8', color: 'rgb(92, 127, 79)' }}>
            LIMITED TIME OFFER
          </div>
