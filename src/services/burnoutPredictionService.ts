...
export interface InterventionPlan {
  ...
  actions: {
    ...
    category: 'self_care' | 'professional_support' | 'workload' | 'social' | 'training';
...
export async function predictBurnoutRisk(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('predict_burnout_risk_zkwv', {
    ...
    return { 
      success: true, 
      data: data as BurnoutRiskAssessment 
    };
  } catch (error) {
...
export function generateInterventionPlan(
  ...
  const plan: InterventionPlan = {
    type: assessment.intervention_urgency === 'immediate' ? 'immediate' : 
          assessment.intervention_urgency === 'urgent' ? 'urgent' :
          assessment.risk_level === 'moderate' ? 'preventive' : 'maintenance',
    actions: [],
    elya_prompts: [],
    resources: []
  };
    ...
    plan.actions.push(
      {
        ...
        description: 'Step away from current assignments for a brief reset period',
        priority: 'critical',
        category: 'self_care',
    ...
    plan.actions.push(
      {
        ...
        description: 'Identify assignments that can be rescheduled or delegated',
        priority: 'high',
        category: 'workload',
  ...
  if (assessment.factors.engagement_days < 3) {
    plan.actions.push({
      ...
      description: 'Restart your daily reflection practice with small, manageable steps',
      priority: 'medium',
      category: 'self_care',
  ...
  if (assessment.factors.energy_trend < 4) {
    plan.actions.push({
      ...
      description: 'Prioritize sleep, nutrition, and movement for energy recovery',
      priority: 'high',
      category: 'self_care',
...
export async function getBurnoutRiskTrend(
  ...
  try {
    ...
    const { data: metrics, error } = await supabase
      ...
      .select('week_of, stress_level, energy_level, burnout_score, high_stress_pattern, recovery_needed')
      .eq('user_hash', userHash)
      .gte('week_of', new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString())
    ...
    subscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (metrics.stress_level > 7 || metrics.burnout_score > 7 || 
             (metrics.energy_level < 4 && metrics.recovery_needed)) {
            
            // Get full risk assessment
            const { data } = await predictBurnoutRisk(userId);
            
            if (data && (data.risk_level === 'critical' || data.risk_level === 'high')) {
              callback({
                risk_level: data.risk_level,
                message: data.risk_level === 'critical' 
