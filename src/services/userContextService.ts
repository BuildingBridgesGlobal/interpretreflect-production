...
class UserContextService {
	...
	async getUserContextForElya(): Promise<ElyaUserContext | null> {
		...
		try {
			...
			if (reflections && reflections.length > 0) {
				...
				if (
					context.user_summary.avg_stress_level >= 8 &&
					context.user_summary.avg_energy_level <= 3
				) {
					context.user_summary.burnout_risk_level = "critical";
				} else if (
					context.user_summary.avg_stress_level >= 7 &&
					context.user_summary.avg_energy_level <= 4
				) {
					context.user_summary.burnout_risk_level = "high";
				} else if (
					context.user_summary.avg_stress_level >= 6 ||
					context.user_summary.avg_energy_level <= 5
				) {
	...
	async saveConversationMessage(
		...
		try {
			...
			// Save anonymous metrics to ZKWV (non-blocking)
			this.saveConversationToZKWV(userId, sender, metadata);
	...
	/**
	 * Save conversation metrics to ZKWV for anonymous analytics
	 * Only saves metadata and metrics, not actual conversation content
	 */
	private saveConversationToZKWV(
		userId: string,
		sender: "user" | "elya",
		metadata: Record<string, any>
	) {
		// Only track user messages for privacy
		if (sender !== "user") return;

		// Extract any numerical metrics from metadata
		const metrics: Record<string, number> = {};

		// Common fields that might be in metadata
		if (metadata.sentiment_score !== undefined) {
			metrics.sentiment_score = Number(metadata.sentiment_score);
		}
		if (metadata.message_length !== undefined) {
			metrics.message_length = Number(metadata.message_length);
		}
		if (metadata.response_time !== undefined) {
			metrics.response_time = Number(metadata.response_time);
		}

		// Save as an AI interaction metric
		saveReflectionZKWV(userId, "ai_interaction", {
			interaction_type: "elya_conversation",
			...metrics
		}).catch(err => {
			console.log("ZKWV conversation save failed (non-critical):", err);
		});
	}
...
Keep responses warm, concise, and actionable. Focus on validation, practical tools, and gentle encouragement.`;
		...
		if (userContext.user_summary?.avg_stress_level) {
			...
			if (stressLevel >= 7) {
				contextualAdditions.push(
					`The user has been experiencing higher stress levels (avg: ${stressLevel.toFixed(1)}/10). Be particularly gentle and focus on immediate stress relief strategies.`,
				);
			} else if (stressLevel <= 4) {
				contextualAdditions.push(
					`The user appears to be managing stress relatively well (avg: ${stressLevel.toFixed(1)}/10). You can explore growth opportunities and prevention strategies.`,
				);
			}
		}
		...
		if (userContext.user_summary?.recent_emotions?.length > 0) {
			const emotions = userContext.user_summary.recent_emotions
				.slice(0, 5)
				.join(", ");
			contextualAdditions.push(
				`Recent emotional patterns include: ${emotions}. Be attuned to these themes in your responses.`,
			);
		}
		...
		if (userContext.user_summary?.burnout_risk_level) {
			...
			if (risk === "high" || risk === "critical") {
				contextualAdditions.push(
					`The user shows ${risk} burnout risk. Prioritize immediate support, boundary-setting, and professional resource recommendations.`,
				);
			}
		}
		...
		if (userContext.user_summary?.effective_strategies?.length > 0) {
			const strategies = userContext.user_summary.effective_strategies
				.slice(0, 3)
				.join(", ");
			contextualAdditions.push(
				`Strategies that have worked well for this user include: ${strategies}. Reference these when appropriate.`,
			);
		}
		...
		if (userContext.recent_reflections?.length > 0) {
			...
			if (recentReflection.stress_level >= 7) {
				contextualAdditions.push(
					`User recently reported high stress (${recentReflection.stress_level}/10) in a ${recentReflection.type} reflection. Check in on this.`,
				);
			}
		}
	...
	async getWellnessInsights(): Promise<any> {
		...
		try {
			const { data, error } = await supabase.rpc(
				"get_wellness_insights_for_elya",
				{
					target_user_id: userId,
				},
			);
	...
	async getTeamInsights(): Promise<any> {
		...
		try {
			const { data, error } = await supabase.rpc("get_team_insights_for_elya", {
				target_user_id: userId,
			});
	...
	async generateRecommendations(): Promise<any> {
		...
		try {
			const { data, error } = await supabase.rpc(
				"generate_recommendations_for_elya",
				{
					target_user_id: userId,
				},
			);
	...
	async analyzeEmotionPatterns(): Promise<any> {
		...
		try {
			const { data, error } = await supabase.rpc(
				"analyze_emotion_patterns_for_elya",
				{
					target_user_id: userId,
				},
			);
	...
	async getAssignmentInsights(): Promise<any> {
		...
		try {
			const { data, error } = await supabase.rpc(
				"get_assignment_insights_for_elya",
				{
					target_user_id: userId,
				},
			);
	...
	async getConversationStats(): Promise<{
		...
		try {
			...
			return {
				...
				avg_session_length:
					sessions.size > 0 ? Math.round(data.length / sessions.size) : 0,
				last_conversation: lastConversation
					? new Date(lastConversation).toISOString()
					: null,
			};
		} catch (error) {
