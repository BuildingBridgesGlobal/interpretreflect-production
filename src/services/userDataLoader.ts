...
export class UserDataLoader {
	static async loadUserData(userId: string): Promise<void> {
		try {
			...
			await UserDataLoader.loadPreferences(userId);

			// Load reflections
			await UserDataLoader.loadReflections(userId);

			// Load body check-in data
			await UserDataLoader.loadBodyCheckIns(userId);

			// Load technique usage
			await UserDataLoader.loadTechniqueUsage(userId);

			// Load burnout assessments
			await UserDataLoader.loadBurnoutAssessments(userId);

			// Load other user-specific data
			await UserDataLoader.loadAffirmations(userId);
			await UserDataLoader.loadSessions(userId);
	...
	private static async loadBurnoutAssessments(userId: string): Promise<void> {
		try {
			...
			if (data && data.length > 0) {
				...
				if (todaysAssessment) {
					localStorage.setItem(
						"todaysBurnoutAssessment",
						JSON.stringify(todaysAssessment),
					);
		...
		try {
			...
			if (!bodyError && bodyAwareness) {
				...
				if (v1Sessions.length > 0) {
					localStorage.setItem(
						"bodyAwarenessSessions",
						JSON.stringify(v1Sessions),
					);
				}
				if (v2Sessions.length > 0) {
					localStorage.setItem(
						"bodyAwarenessV2Sessions",
						JSON.stringify(v2Sessions),
					);
				}
			}
