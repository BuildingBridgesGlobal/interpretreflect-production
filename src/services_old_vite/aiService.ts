...
import { type ElyaUserContext, userContextService } from "./userContextService";
...
const defaultConfig: AIConfig = {
	provider:
		(import.meta.env.VITE_AI_PROVIDER as
			...
			| "openrouter"
			| "agenticflow"
			| "simulated") || "agenticflow",
	apiKey:
		import.meta.env.VITE_OPENAI_API_KEY ||
		import.meta.env.VITE_OPENROUTER_API_KEY ||
		import.meta.env.VITE_AGENTICFLOW_API_KEY,
	model: import.meta.env.VITE_AI_MODEL || "openrouter/sonoma-sky-alpha",
	agentId:
		import.meta.env.VITE_AGENTICFLOW_AGENT_ID ||
		"a1cab40c-bcc2-49d8-ab97-f233f9b83fb2",
...
class AIService {
	...
	private async initializeWithContext() {
		try {
			...
			const contextualPrompt =
				await userContextService.generateContextualSystemPrompt();
	...
	private async getAgenticFlowResponse(userMessage: string): Promise<string> {
		const agentId =
			this.config.agentId || "a1cab40c-bcc2-49d8-ab97-f233f9b83fb2";
		...
		try {
			...
			const enhancedMetadata = {
				...
				...(this.userContext?.user_summary && {
					user_profile: {
						...
						recent_emotions:
							this.userContext.user_summary.recent_emotions?.slice(0, 5),
						common_challenges:
							this.userContext.user_summary.common_challenges?.slice(0, 3),
						effective_strategies:
							this.userContext.user_summary.effective_strategies?.slice(0, 3),
					},
				}),
			};
			...
			const response = await fetch(
				`https://api.agenticflow.ai/v1/agents/${agentId}/chat`,
				{
					method: "POST",
					headers: {
			...
			if (!response.ok) {
				...
				const embedResponse = await fetch(
					"https://agenticflow.ai/api/v1/agent/chat",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
				...
				if (embedResponse.ok) {
					...
					return (
						data.response ||
						data.message ||
						this.getContextualSimulatedResponse(userMessage)
					);
				}
	...
	private async getAIResponse(): Promise<string> {
		...
		const response = await fetch(endpoint, {
			...
			body: JSON.stringify({
				model:
					this.config.model ||
					(isOpenRouter ? "openrouter/sonoma-sky-alpha" : "gpt-3.5-turbo"),
	...
	private getContextAwareResponse(
		userInput: string,
		context: ElyaUserContext,
	): string {
		...
		if (
			(input.includes("stress") || input.includes("overwhelm")) &&
			summary.avg_stress_level >= 7
		) {
		...
		if (
			(input.includes("tired") || input.includes("exhausted")) &&
			summary.avg_energy_level <= 4
		) {
		...
		if (
			summary.burnout_risk_level === "high" ||
			summary.burnout_risk_level === "critical"
		) {
			const riskMessages = {
				...
				critical:
					"Your recent patterns suggest critical burnout risk. I'm genuinely concerned about your wellbeing right now.",
			};
