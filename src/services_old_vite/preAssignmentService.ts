...
class PreAssignmentService {
	...
	async saveResponses(
		userId: string,
		responses: Partial<PreAssignmentResponseV2>,
		...
		try {
			...
			return {
				success: false,
				error:
					error instanceof Error ? error.message : "Failed to save responses",
			};
		}
	}
	...
	async getResponses(
		userId: string,
		assignmentId?: string,
	): Promise<PreAssignmentResponseV2 | null> {
		try {
	 ...
	 */
	async getAllCompletedResponses(
		userId: string,
	): Promise<PreAssignmentResponseV2[]> {
		try {
		...
		try {
			...
			if (cognitiveLoads.length > 0) {
				const avgCognitiveLoad =
					cognitiveLoads.reduce((a, b) => a + b, 0) / cognitiveLoads.length;

				if (avgCognitiveLoad >= 7) {
					insights.strengths.push("Consistently high mental readiness");
				} else if (avgCognitiveLoad <= 4) {
					insights.areas_for_development.push(
						"Mental preparation and cognitive load management",
					);
					insights.recommendations.push(
						"Consider implementing pre-assignment mindfulness routines",
					);
				}
				...
				if (cognitiveLoads.length >= 3) {
					const recentAvg =
						cognitiveLoads.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
					const olderAvg =
						cognitiveLoads.slice(-3).reduce((a, b) => a + b, 0) / 3;
			...
			if (roleBoundaries.length > 0) {
				const avgRoleBoundaries =
					roleBoundaries.reduce((a, b) => a + b, 0) / roleBoundaries.length;
				...
				if (avgRoleBoundaries >= 8) {
					...
					insights.recommendations.push(
						"Review professional boundary guidelines and practice role clarification scripts",
					);
				}
			}
			...
			if (mostCommonEmotions.length > 0) {
				insights.patterns.push(
					`Common emotional states: ${mostCommonEmotions.join(", ")}`,
				);
				...
				const positiveCount = mostCommonEmotions.filter((e) =>
			...
			if (consistentStrategies.length > 0) {
				insights.strengths.push(
					`Consistent use of effective strategies: ${consistentStrategies.slice(0, 2).join(", ")}`,
				);
				insights.progress_score += 20;
			}

			// Calculate overall progress score
			insights.progress_score = Math.min(
				100,
				insights.progress_score + responses.length * 5,
			);

			// Generate personalized recommendations
			if (insights.areas_for_development.length > 0) {
				insights.recommendations.push(
					"Focus on identified development areas in upcoming assignments",
				);
			}

			if (responses.length >= 5) {
				insights.recommendations.push(
					"Consider creating a personal best practices document based on your successful strategies",
				);
			}
	...
	async linkToDebrief(
		...
		postDebriefId: string,
	): Promise<boolean> {
		try {
			const { error } = await supabase.from("assignment_links").insert({
