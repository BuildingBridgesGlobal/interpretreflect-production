...
class Analytics {
  ...
  trackUserAction(action: string, section: string, details?: Record<string, any>) {
    this.track({
      event: 'user_action',
      category: 'engagement',
...
export const trackUserFlow = (action: string, location: string, details?: Record<string, any>) => {
  analytics.trackUserAction(action, location, details);
};

export const trackWellnessTool = (toolName: string, duration?: number, completed?: boolean) => {
  analytics.trackToolUsage(toolName, duration, completed);
};

export const trackSubscriptionFlow = (step: string, details?: Record<string, any>) => {
