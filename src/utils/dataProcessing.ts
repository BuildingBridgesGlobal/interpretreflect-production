 ...
 */
export const getReflectionSummary = (
	reflection: Record<string, unknown>,
): string => {
	const type = (reflection.type as string) || "Reflection";
	const date = reflection.date
		? new Date(reflection.date as string).toLocaleDateString()
		: "Recent";
 ...
 */
export const getMostCommonRiskLevel = (
	items: BurnoutData[],
	...
	return (mostCommon?.[0] || "moderate") as
		| "low"
		| "moderate"
		| "high"
		| "severe";
	...
	return [...reflections]
		.sort((a, b) => {
			const dateA = new Date(
				(a.timestamp as string) || (a.date as string) || 0,
			);
			const dateB = new Date(
				(b.timestamp as string) || (b.date as string) || 0,
			);
