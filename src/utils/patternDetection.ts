...
export const DETECTION_RULES: DetectionRule[] = [
  // Medical Assignment Fatigue Pattern
  {
    ...
    condition: (data: UserData) => {
      const medicalAssignments = data.assignments.filter(a => 
        a.type === 'medical' && 
        new Date().getTime() - a.timestamp.getTime() < 7 * 24 * 60 * 60 * 1000
      );
      
      const fatigueAfterMedical = medicalAssignments.filter(a => {
        const emotionsAfter = data.emotions.filter(e =>
          e.timestamp.getTime() > a.timestamp.getTime() &&
          e.timestamp.getTime() < a.timestamp.getTime() + 4 * 60 * 60 * 1000 && // within 4 hours
          (e.emotion === 'exhausted' || e.emotion === 'stressed' || e.emotion === 'overwhelmed')
        );
        return emotionsAfter.length > 0;
      });
      
      return fatigueAfterMedical.length / Math.max(medicalAssignments.length, 1) > 0.6;
    },
    threshold: 3,
  ...
  // Missed Reset Consequence Pattern
  {
    ...
    condition: (data: UserData) => {
      ...
      const recentMissed = missedResets.filter(r =>
        new Date().getTime() - r.timestamp.getTime() < 3 * 24 * 60 * 60 * 1000
      );
      
      if (recentMissed.length === 0) return false;
      
      const stressAfterMissed = data.emotions.filter(e => {
        const afterMissedReset = recentMissed.some(r =>
  ...
  // Legal Assignment Anxiety Pattern
  {
    ...
    condition: (data: UserData) => {
      const legalAssignments = data.assignments.filter(a => 
        a.type === 'legal' || a.type === 'court'
      );
      
      const anxietyBeforeLegal = legalAssignments.filter(a => {
        const emotionsBefore = data.emotions.filter(e =>
          e.timestamp.getTime() < a.timestamp.getTime() &&
          e.timestamp.getTime() > a.timestamp.getTime() - 24 * 60 * 60 * 1000 &&
          e.emotion === 'anxious' &&
          e.intensity >= 3
        );
      ...
      return anxietyBeforeLegal.length / Math.max(legalAssignments.length, 1) > 0.7;
    },
    threshold: 2,
    generateNudge: (pattern) => ({
      id: `nudge-${pattern.type}-${Date.now()}`,
  ...
  // Afternoon Energy Dip Pattern
  {
    ...
    condition: (data: UserData) => {
      ...
      const totalAfternoons = new Set(
        data.emotions
          .filter(e => e.timestamp.getHours() >= 14 && e.timestamp.getHours() <= 16)
          .map(e => e.timestamp.toDateString())
      ).size;
  ...
  // Effectiveness Pattern
  {
    ...
    condition: (data: UserData) => {
      const effectiveActions = data.wellnessActions.filter(w =>
        w.effectiveness && w.effectiveness >= 4
      );
      
      if (effectiveActions.length < 5) return false;
      
      const actionCounts = effectiveActions.reduce((acc, action) => {
        acc[action.category] = (acc[action.category] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
      
      const topCategory = Object.entries(actionCounts)
        .sort(([, a], [, b]) => b - a)[0];
...
export class PatternDetectionEngine {
  ...
  private isNudgeActive(nudge: PatternNudge): boolean {
    return this.activeNudges.some(active =>
      active.title === nudge.title &&
      active.message === nudge.message
    );
  }
  ...
  getPersonalizedRecommendations(): string[] {
    ...
    for (const pattern of patterns.slice(0, 3)) {
      if (pattern.pattern.includes('medical')) {
        recommendations.push('Schedule boundary-setting time after medical assignments');
        ...
        recommendations.push('Practice confidence exercises before court assignments');
      }
    }
  ...
  cleanupExpiredNudges(): void {
    ...
    this.activeNudges = this.activeNudges.filter(nudge => {
      if (nudge.expiresIn) {
        const expirationTime = new Date(nudge.id.split('-').pop() || '').getTime() + 
  ...
  getActiveNudges(): PatternNudge[] {
    ...
    return this.activeNudges.sort((a, b) => 
