	...
	window.customElements.define = (
		...
		try {
			...
			if (error instanceof DOMException && error.name === "NotSupportedError") {
				...
				if (!isExtensionElement) {
					console.warn(
						`Failed to define custom element "${name}":`,
						error.message,
					);
				}
			} else {
...
if (typeof window !== "undefined" && window.customElements) {
	...
	if (typeof window !== "undefined") {
		...
		if (document.querySelector("[data-grammarly-shadow-root]")) {
			console.info(
				"Grammarly extension detected - custom element conflicts have been handled.",
			);
		}
		...
		setTimeout(() => {
			...
			if (window.customElements.get("mce-autosize-textarea")) {
				console.info(
					"TinyMCE or similar editor extension detected - conflicts handled.",
				);
			}
		}, 1000);
	}
