...
export class SessionManager {
  ...
  private checkSession(): void {
    ...
    const timeUntilTimeout = SECURITY_CONFIG.session.timeout - timeSinceActivity;

    if (timeUntilTimeout <= 0) {
      this.endSession('TIMEOUT');
    } else if (timeUntilTimeout <= SECURITY_CONFIG.session.warningTime && !this.warningShown) {
      this.showWarning();
...
export class AuditLogger {
  ...
  static clearOldLogs(daysToKeep: number = SECURITY_CONFIG.audit.retentionDays): void {
...
export class RoleManager {
  ...
  static getUserRole(userId: string): string {
    return this.userRoles.get(userId) || SECURITY_CONFIG.rbac.defaultRole;
  }

  static hasPermission(userId: string, permission: string): boolean {
    const userRole = this.getUserRole(userId);
    const roleConfig = SECURITY_CONFIG.rbac.roles[userRole as keyof typeof SECURITY_CONFIG.rbac.roles];
