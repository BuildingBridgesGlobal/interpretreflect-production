...
console.log('🚀 Standardizing close behavior for all reflection components...\n');
...
reflectionComponents.forEach(filename => {
  ...
  if (content.includes('setTimeout(() => {') && content.includes('onComplete(')) {
  ...
  saveSuccessPatterns.forEach(pattern => {
    if (pattern.test(content)) {
      content = content.replace(pattern, (match) => {
        ...
        if (!nextLines.includes('onComplete(') && !nextLines.includes('onClose()')) {
  ...
  if (modified) {
    ...
    if (content.includes('onComplete') && content.includes('onClose')) {
      ...
      const hasProperClose =
        ...
        (content.includes('onClose()') || content.includes('setTimeout(() => {\n        onClose()'));
