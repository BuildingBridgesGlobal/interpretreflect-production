...
function createAsyncRefreshVersion(modalName, setterName, usesSaveReflection = false) {
  if (usesSaveReflection) {
    return `      {${modalName} && (
        <${modalName.replace('show', '')}
          onComplete={async (results) => {
            // Save reflection
            saveReflection('${modalName.replace('show', '').replace(/([A-Z])/g, ' $1').trim()}', results);
            ${setterName}(false);
    ...
    return `      {${modalName} && (
        <${modalName.replace('show', '')}
          onComplete={async (data) => {
            console.log('${modalName.replace('show', '').replace(/([A-Z])/g, ' $1').trim()} Results:', data);
            ${setterName}(false);
...
const modalsToUpdate = [
  {
    ...
    replacement: createAsyncRefreshVersion('showMentoringReflection', 'setShowMentoringReflection')
  },
  {
    pattern: /\{showRoleSpaceReflection && \([^}]+\}\)/,
    replacement: createAsyncRefreshVersion('showRoleSpaceReflection', 'setShowRoleSpaceReflection')
  },
  {
    pattern: /\{showDirectCommunicationReflection && \([^}]+\}\)/,
    replacement: createAsyncRefreshVersion('showDirectCommunicationReflection', 'setShowDirectCommunicationReflection')
  },
  {
    pattern: /\{showInSessionSelfCheck && \([^}]+\}\)/,
    replacement: createAsyncRefreshVersion('showInSessionSelfCheck', 'setShowInSessionSelfCheck', true)
  },
  {
    pattern: /\{showInSessionTeamSync && \([^}]+\}\)/,
    replacement: createAsyncRefreshVersion('showInSessionTeamSync', 'setShowInSessionTeamSync', true)
  }
];
