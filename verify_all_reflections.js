...
reflectionComponents.forEach(filename => {
  ...
  const usesDirectApi = content.includes('directInsertReflection') ||
                        content.includes('directSupabaseApi');

  // Check for problematic Supabase client usage
  const usesSupabaseClient = content.includes('from \'@supabase/supabase-js\'') ||
                             content.includes('from "@supabase/supabase-js"');

  // Check for hanging growth insights call (not commented out)
  const hasActiveGrowthInsights = content.match(/^[^/]*updateGrowthInsightsForUser\(/m);

  // Check if it has a save function
  const hasSaveFunction = content.includes('handleSave') ||
